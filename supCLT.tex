%%----Versions-----------------------------
% supCLT0109.tex 2018/01/14 by Yanxia
...
\end{equation}	
  ...
  %  According to the Subsection A.1, the branching mechanism $\psi$ can be extended on
...
%\tilde{m} is changed to \widetilde{m} withput marking.
    \widetilde{m}[f]
...
%the value of the L\'evy exponent of the infinite divisible random variable
...
%end new
%The following is moved from below with modification.
   Define
 \begin{equation}\begin{split}
 \label{parameter_mk}
      m_t[f]
...
\end{lem}
 ...
 Recall the definition of $\widetilde{m}$ in \eqref{tilde-m}.
  ...
  \begin{lem}(\cite[Lemma 4.2]{MM})
   % According to \cite[Lemma 4.2]{MM}, if $f \in \mathcal{P}$ satisfies $\alpha\beta=\kappa_f b(1+\beta)$,
...
\label{para: critical case}
 ...
 \lim_{t\rightarrow \infty}\frac{1}{t}\sum_{k=0}^{\lfloor t \rfloor}m_k[f]=\langle\eta(-i\phi)^{1+\beta},\varphi\rangle=\widetilde{m}[f],
\end{split}\end{equation}
    where
\[
    \phi
...
%    $\theta \mapsto \exp(\widetilde m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable, provided $\alpha\beta=\kappa_f b(1+\beta)$;
If $\alpha\beta=\kappa_f b(1+\beta)$,  $\theta \mapsto \exp(\widetilde m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable.
\end{enumerate}
%\end{cor}
\end{prop}
\begin{proof}
...
%	For all $n\geq 0$ and $g\in \mathcal C_l$
%expressed by \eqref{discompose},
...
In this section, we will introduce a small value probability result of supercritical continuous-state branching process with general branching mechanism and %control
...
\]
    ...
    %For $t$ large enough, according to \eqref{CSBP: int} and the fact that $0<k_t^{-1}e^{-\alpha t} < v_t(k_t^{-1}e^{-\alpha t})< \bar v$, we can verify the following:
...
First we introduce an criterion of $(1+\beta)$-stable distribution.
\begin{lem}
...
{\bf( Remark: State \cite[Lemma 2.8]{MM} as a Lemma in our paper)}
\[
     |P_t g_k|\leq e^{-bt}h,
...
\begin{proof}
    ...
    \end{align*}
 %   According to Corollary \ref{cor: alpha stable rv} and Corollary \ref{cor: used in next corollary},
...
\begin{align*}
    ...
    \\& \leq C'e^{-\delta (t-k)}.
\end{align*}
%Now we can use the same argument as in Step 3 in Section \ref{sec: proof of thm 1.3}
...
%[\langle g_p, X_{t+n-1}\rangle],
[\langle g_n, X_{t+n-1}\rangle],
...
\[
     ...
     \quad t\geq 0, n\geq 1.
\]
{\bf Remark: State the above result as a Lemma}
 Choose $\gamma\in(0,\beta)$ small enough such that $\alpha \gamma < b < (1+\gamma)b$.
 %According to Lemma \ref{lem: control moment} (3) with $\kappa=1$,
 Using Lemma \ref{lem: control moment} (3) with $\kappa=1$,
 there exists $C>0$ such that
\begin{align}
...
Using similar arguments in the proof of step 4 of Lemma \ref{lem: mainlemma}, we can show that for each $\epsilon< \alpha$ there exist $C,\delta>0$ such that
...
Step 2.   Fix $t\geq 0$, $m\in \mathbb{N}$ and $\theta\in \mathbb{R}$.
    ...
    \end{align*}
    %According to step 1 and corollary \ref{cor: alpha stable rv 1}
...
\end{align}
%Step 1. Similar to the proof of step 1 in section \ref{sec: proof of thm 1.3},
...
\end{align*}
\end{comment}
%Since $\eta_1 < \eta_0$,  Similar to the proof of step 2 in section \ref{sec: proof of thm 1.3},
