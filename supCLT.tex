...
%---Contents-----------------------
\begin{abstract}
    Consider a super-OU process $\{X_t, t\geq 0\}$ with branching mechanism $\psi(z)=-\alpha z +z^{1+\beta}$, where $\alpha >0$ and $\beta\in (0,1)$. For each testing function $f$ 
%    with polynomial growth, 
of at most polynomial growth,
    denote by $\kappa_f$ the order of $f$ in term of the spectral decomposition of $f$ 
%    with respect to the mean semigroup of $X$. 
in terms of the spectrum of the the mean semigroup of $X$. 
    Conditioned on no-extinction, we establish some spatial central limit theorems for $\langle f, X_t \rangle$ in three different regimes: In the small branching rate case, $\alpha\beta< \kappa_f b(1+\beta)$, $\langle f,X_t\rangle/\|X_t\|^{\frac{1}{+\beta}}$ 
%    convergences weakly. 
converges weakly.
    In the critical branching rate case, $\alpha\beta= \kappa_f b(1+\beta)$,  $\langle f,X_t\rangle/(t\|X_t\|)^{\frac{1}{+\beta}}$ 
%    convergences weakly. 
converges weakly.
    In the large branching rate case,  $\alpha\beta> \kappa_f b(1+\beta)$, $e^{-(\alpha-\kappa(f)b)t}\langle f,X_t\rangle$ 
%   convergences a.s and in $L^{1+\gamma}$, with $\gamma \in (0,\beta)$.
converges almost surely and in $L^{1+\gamma}$, for any $\gamma \in (0,\beta)$.
\end{abstract}
\maketitle
...
\end{equation}	
	...
	The existence of superprocesses is well known, see \cite{EB} for instance.
...
$$H_t^p:= e^{-(\alpha-|p|b)t}\langle\phi_p,X_t\rangle,\quad t\geq 0.$$
 ...
 \begin{thm}\label{Theorem11}
     ...
     Moreover, if $f$ is twice differentiable and 
%     its twice derivative $D^2 f$ satisfies $|D^2 f| \in \mathcal{P}$, then the convergence is almost sure.
all its second order partial derivatives are in $\mathcal{P}$, then we also have almost sure convergence.
 \end{thm}
For any $t>0$, let $H_t:=e^{-\alpha t}\|X_t\|$, then $H_t$ is equal to $H_t^0$ and is a non-negative martingale with limit $H_{\infty}:=\lim_{t\rightarrow\infty}H_t$,  $\mathbb{P}_{\mu}$-a.s. and in $L^{1+\gamma}(\mathbb{P}_{\mu})$.
 \begin{rem}
    ...
    Moreover, if $f$ is twice differentiable and 
%    $|D^2 f| \in \mathcal{P}$, then the convergence is almost sure.
all its second order partial derivatives are in $\mathcal{P}$, then we also have almost sure convergence.
 \end{rem}
...
$$\mathbb{E} [e^{i\theta \eta_1}]=\exp(\tilde{m}[\theta f]), \quad \theta\in \mathbb R.$$
\end{thm}
...
\begin{lem}\label{lem: control of Wt}
 ...
 $$
 \|H_t-H_s\|_{\mathbb{P}_{\mu};1+\gamma}\leq C 
 %e^{-\alpha s/2}.
 e^{-\frac{\alpha \gamma}{1+\gamma}s}.
 $$
\end{lem}
...
\end{align}
    ...
    Step 1. We will show that there exists a $C>0$ such that for all $k \geq 0$ and $t\geq k+1$,
\begin{align}
\label{thm121}
...
\end{align}
    ...
    Let $C_1$ be the constant in  Lemma \ref{lem: control of Wt} corresponding to this $\gamma$ and $C_2$ be the constant in Lemma \ref{lem: control of XT}. 
    ({\bf The next display needs to be revised because of the corrected statement of Lemma 2.9.
    The whole proof needs to be changed accordingly.})
...
\end{align}
    ...
    Step 2. We will show that there exists constant $C > 0$ such that for all 
    $0\leq k\leq t-\ln t$ with $t$ large enough,
\begin{align}
\label{thm122}
...
\end{equation}
    Note that for all $0 < x< y$, we have for $t$ large enough,
\[
    1-e^{-(y-x)t} 
    \geq e^{-xt},
\]
...
\]
Thus for $0\leq k\leq t-\ln t$ with $t$ large enough,
\begin{align*}
    H_t
...
\end{align*}
    Therefore, for $0\leq k\leq t-\ln t$ with $t$ large enough,
\begin{align*}
...
\begin{align*}
    ...
    \quad \text{ on } \mathcal D_{t,k}.
\end{align*}
    Thus, there is a constant $C\geq 0$ such that  for $0\leq k\leq t-\ln t$ with $t$ large enough,
\begin{align}
\label{ineq: control of Kkt}
     K_{t,k}
     \leq C e^{-\frac{3\alpha}{32}(t-k-1)}.
\end{align}
    Now fix $\gamma \in (0, \beta)$ close enough to $\beta$ so that 
\[
    \frac{1}{1+\gamma} - \frac{1}{1+\beta} 
    \leq \frac{1}{32}.
\]
    From the condition $\alpha\beta=\kappa_f(1+\beta)b$, we get that 
\begin{align}
...
\end{align}
    According to Lemma \ref{lem: control of mgtrs}, \eqref{eq: control of Ykt}, \eqref{ineq: control of Kkt} and the above, there exist constants $C,C'>0$ such that  for $0\leq k\leq t-\ln t$ with $t$ large enough,
\begin{align}
\label{eq: Y in D}
    &\mathbb{\tilde{P}}_{\mu}\big[|Y_{t,k}|\mathbf{1}_{\mathcal{D}_{t,k}}\big]
...
\]
    ...
    for $0\leq k\leq t-\ln t$ with $t$ large enough,
\[
    \tilde{\mathbb P}_\mu\big[|Y_{t,k}|\big] 
    \leq C e^{-\delta(t-k)}.
\]
    Therefore, there exist $C, \delta > 0$ and $C'$ such that for $t$ large enough,
\begin{align}
...
\label{ineq: control of I1tb}
    ...
    \quad t\geq t_0,
\end{align}
	Let $\gamma\in (0,\beta)$ be close enough to $\beta$ so that
\[
...
\]
	...
	Using \eqref{eq: prob of Dtkc}, \eqref{ineq: control of Kkt} and an argument similar to that used in proving \eqref{eq: control of Ykt}, we get that there exist $C,C'>0$ such that for $0 \leq k \leq t - \ln t$ with $t$ large enough,
\begin{align*}
...
\label{eq: condition for supercritical}
    ...
    =\frac{\alpha \gamma-\kappa_f(1+\gamma)b}{1+\gamma}.
\end{align}
	Using this, we have that there exists $C > 0$ such that for $0 \leq k \leq t - \ln t$ with $t$ large enough,
\begin{align*}
    &\mathbb{\tilde{P}}_{\mu}\big[|Y'_{t,k}|\big]
...
\label{eq: path integration representation of h}
    ...
    = h(z) - h(z_0).
\end{align}
    The rest of the proof follows the argument in \cite[Section 10.14]{Rudin1987Real} and we omit the details.
\end{proof}
...
\end{itemize}
    We refer to $(\mathbb N_x)_{x\in E}$ as the Kuznetsov measures of $X$.
\subsection{}
...
\end{equation}
    ...
    It is known from \cite[Proposition 2.27]{Li2011Measure-valued} and \cite[Theorem 2.7]{Kyprianou2014Fluctuations} that for all $t > 0$, $\mu \in \mathcal M_E^1$ and $f\in b\mathscr B^+_E$,
\begin{equation}
...
\[
    ...
    \in \mathbb C
\]
    is well defined. First, we only assume that $f\in L^1(\xi)$.
    Noticing that
\[
...
\begin{proof}[Proof of Proposition \ref{prop: complex FKPP-equation}]
    ...
    Fix $t>0, r\in [0,t), x\in E$ and $g\in b\mathscr B_E^{++}$.
...
&\int_{[0,t]\times \mathbb W}|e^{i w_{t-s}(f)} - 1| \mathbf m(\xi, ds,dw)
    ...
    < \infty.
\end{align}
    AUsing this, Fubini's theorem, \eqref{eq: N and characteristic exponent} and \eqref{eq: -v has positive real part} we have $\mathbf Q^{(g,t)}_{x}$-almost surely,
\begin{align}
...
\end{equation}
    Taking $r = 0$, we get that \eqref{eq: complex FKPP-equation} and \eqref{eq: complex FKPP-equation with FK-transform} are true if $f\in b\mathscr B_E$.
