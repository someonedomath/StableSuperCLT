%%%----Versions-----------------------------
% supclt1213.tex 2018/12/13 by Yanxia
...
\begin{abstract}
    ...
    denote by $\kappa_f$ the order of $f$ in
the spectral decomposition of $f$ in terms of the spectrum of the the mean semigroup of $X$.
...
\end{equation}
 ...
 %   For each $p = (p_k)_{k = 1}^d \in \mathbb{N}_0^{d}$,
  For each $p = (p_k)_{k = 1}^d \in \mathbb{Z}_+^{d}$,
    let $|p| \added{ :}=\sum_{k=1}^d p_k$, $p!:= \prod_{k= 1}^d p_k !$ and $\partial x^p:= \prod_{k = 1}^d\partial x_k^{p_k}$.
...
\subsection{}
    Let $X$ be the super OU-process discussed in Subsection \ref{ss:1.1}.
...
\end{equation}
   ...
   Write $\psi_0(z) := \psi(z) + \alpha z=z^{1+\beta}$.
  %  Then $\psi'_0(z) = \psi'(z) + \alpha=(1+\beta)z^{\beta}$.
   Then $\psi'_0(z) = \psi'(z) + \alpha=(1+\beta)z^{\beta}=(1+\beta)z^\beta$.
...
$$H_t^p:= e^{-(\alpha-|p|b)t}\langle\phi_p,X_t\rangle,\quad t\geq 0.$$
  For any $\gamma\in (0, \beta)$, we will prove (see Lemma \ref{lemma26}) that, if $\alpha\beta>|p|b(1+\beta)$, $H_t^p$ is a $\mathbb{P}_{\mu}$-martingale bounded in
   $L^{1+\gamma}(\mathbb{P}_{\mu})$ for any $\mu\in \mathcal M_c(\mathbb R_d)$,
...
any $\gamma\in (0, \beta)$ and any $\mu\in \mathcal M_c(\mathbb R_d)$,
 ...
 \end{thm}
For any $t>0$, let $H_t:=e^{-\alpha t}\|X_t\|$, then $H_t$ is equal to $H_t^0$ and is a non-negative martingale with limit $H_{\infty}:=\lim_{t\rightarrow\infty}H_t$,  $\mathbb{P}_{\mu}$-a.s. and in $L^{1+\gamma}(\mathbb{P}_{\mu})$, for all $\gamma\in (0, \beta)$ and $\mu\in \mathcal M_c(\mathbb R_d)$.
 \begin{rem}
...
\section{Preliminaries}
    ...
    For any $a > 0$, define $A^{\times a}f(x):= (Af(x))^a$ for all $f\in \mathcal D_A$ and $x\in \mathbb R^d$.
...
\]
    Then  $\{A_\nu: \nu \text{ is  a probability measure on } (\Lambda, \mathscr F)\}$ is uniformly $\gamma$-controllable on $\mathcal D$.
\item[(2)]
...
\end{lem}
\begin{proof}
    (1). Let $(R,S)$ be the $\gamma$-control-pair of $(A_\lambda)_{\lambda\in\Lambda}$ on $\mathcal{D}$. For all $f \in \mathcal{D}$, $x\in \mathbb R^d$ and  probability measure $\nu$ on $(\Lambda, \mathscr F)$,
\[
...
$$
	\mathcal I_{a_1}^{a_2} Y
  := \mathbb P_\mu[Y|\mathscr F_{a_2}] - \mathbb P_\mu[Y|\mathscr F_{a_1}].
$$
...
\begin{lem}\label{lem: temp}
	For all $h \in \mathcal P^+$ and $\mu \in \mathcal M_c(\mathbb{R}^d)$, there exists a constant $C > 0$ such that for all $\kappa \in \mathbb Z_+ $ and $0\leq r\leq s\leq t$ with $s-r \leq 1$, we have
\[
...
\label{lem: control of mgtrs}
	For all $h \in \mathcal P$, $\mu \in \mathcal M_c(\mathbb{R}^d)$ and $\gamma\in (0, \beta)$, there exists a constant $C > 0$ such that for each $\kappa \in \mathbb Z_+$ and $0\leq r\leq s\leq t$ with $s-r \leq 1$, we have
\[
...
Using Lemma \ref{lem: control of mgtrs} with $g=1$ and $k=0$,
...
From now on, we write $\mathbb{\tilde{P}}_{\mu}=\mathbb{P}_{\mu}(\cdot|D^c)$.
\begin{lem}
...
\]
    Recall that the extinction probability of the superprocess is $\mathbb{P}_{\mu}(D)=e^{-\alpha^*\|\mu\|}$, where $\alpha^*=\alpha^{1/\beta}$ is the larger root of $\psi(z)=0$.
...
%$\lambda = f(t)^{-1} \in [0,1]$
$\lambda = f(t)^{-1} \in (0,1]$
back to \eqref{eq: inequality before take lambda} and using the above inequality, we get the desired result.
\end{proof}
...
\end{align}
In fact, from the definitions of $U_1, \tilde U_1$ and $Z_1$, we have that for all
$k\geq 0, t\geq k+1$,
\begin{align}
\label{eq: need1}
...
Step 2.  We will show that there exists a $C>0$ such that for all $k\geq 0$, $t\geq k+1$ and $\epsilon> 0$,
    ...
    \label{lemma31q}
      %J_1(k,t,\epsilon)
      J_1(t,k,\epsilon)
      \leq C\exp\Big\{-\Big(\alpha\frac{\beta}{1+\beta}-\epsilon\frac{1+2\beta}{1+\beta}\Big)(t-k-1)\Big\}.
    \end{align}
...
\begin{align}\label{ineq: control of J3}
 %   J_3(k,t,\epsilon)\leq Ce^{-\delta (t-k-1)}.
 J_3(t,k,\epsilon)\leq Ce^{-\delta (t-k-1)}.
\end{align}
...
%	\leq \exp({-\alpha^* e^{(\alpha - \epsilon)t}}).
\leq \exp({-\alpha^* \|\mu\|e^{(\alpha - \epsilon)t}}).
\end{align}
According to Lemma \ref{lem: control of XT}, there is a constant $C>0$ such that
for all $t\geq 0, \epsilon >0$,
\begin{align}
	\mathbb P_\mu(A_t(\epsilon)^c,D^c) = \mathbb P_\mu(D^c) \tilde{\mathbb P}_\mu(\|X_t\|\leq e^{(\alpha - \epsilon) t}) \leq \mathbb P_\mu(D^c)C e^{-\epsilon \beta t}.
\end{align}
	Combining these, we get the desired result in step 4.

	Step 5.  Combining the results in Steps 1-4, noticing that, if $\epsilon>0$ is chosen small enough then $J_{i}, i = 1,2,3$ converge to $0$ exponentially fast as $t\rightarrow\infty$,
...
%for all $T\geq 0$,
for all $t\geq 0$,
%$n \in \{0,...,\lfloor t \rfloor\}$ and $(\theta_0,...,\theta_n)\in \mathbb{R}^{n+1}$
...
\label{sec: proof of thm 1.3}
    ...
    \begin{align*}
     ...
     %   \frac{\langle P^\alpha_tf,X_0\rangle}{(t\|X_t\|)^{\frac{1}{1+\beta}}}.
         \frac{\langle T^\alpha_tf,X_0\rangle}{(t\|X_t\|)^{\frac{1}{1+\beta}}}.
        \\&=:I_1(t)+I_2(t) + I_3(t),
        \quad t\geq t_0.
    \end{align*}
    Define
...
\label{thm121}
    ...
    \leq C e^{-\delta (t-k)}.
\end{align}
    In fact, let $C_1$ be the constant in  Lemma \ref{lem: control of Wt} corresponding to this fixed $\gamma$ and $C_2$ be the constant in Lemma \ref{lem: control of XT}.
...
\end{align}
   % According to Lemma \ref{lem: control of mgtrs}, \eqref{eq: control of Ykt}, \eqref{ineq: control of Kkt}, 
...
\begin{align}
    ...
    = C'\frac{1}{t^{\delta}},
\end{align}
%    giving the desired result in this step.
which implies \eqref{domi-difference of Is}  by \eqref{ineq: control of I1t}.
...
\leq \frac{\langle |T^\alpha_tf|,X_0\rangle}{(t\|X_t\|)^{\frac{1}{1+\beta}}}
...
\]
	In order to do this, 
%we choice a $\gamma_0 \in (0,\beta)$ close enough to $\beta$ such that, 
we choose a $\gamma_0 \in (0,\beta)$ close enough to $\beta$ such that, 
for each $\gamma \in [\gamma_0, \beta)$, we have $a < \frac{\alpha\gamma}{b(1+\gamma)}$.
...
\[
    ...
    \leq C  e^{-(\alpha - a b)t}e^{\frac{\alpha}{1+\gamma}t}
     ...
     \quad t\geq 0;
\]
{\bf Question: $J_t$ should be $e^{-(\alpha - ab)t} \langle g, X_t\rangle$, right?}
\item
...
\[
	...
	\quad t\geq 0.
\]
{\bf Question: the above $J_t$ should be replaced by $e^{-(\alpha - ab)t} \langle g, X_t\rangle$, right?}
\end{enumerate}
...
%\frac{\langle P_t^\alpha f, X_0\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}
\frac{\langle T_t^\alpha f, X_0\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}
...
\end{align*}
 ...
 \]
    ...
    Taking $\theta_k=(e^{\alpha})^{-\frac{1}{1+\beta}} \theta $ and $n={\lfloor t-\ln t \rfloor}$ in 
  %  Corollary \ref{corollary31}, 
  Proposition \ref{corollary31},
    we get that there exist $C,\delta > 0$ such that
\begin{align*}
...
\begin{align}
	...
	%\leq \frac{\langle |P^\alpha_tf|,X_0\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}
     \leq \frac{\langle |T^\alpha_tf|,X_0\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}
...
\subsection{Analytic facts}
    In this subsection, we present some some analytic facts.
\begin{lem}
...
\label{eq: estimate of deriavetive of v(theta)}
    ...
    %\leq (P^\beta_t |f|)(x) \sup_{\theta \in K}|\theta| < \infty.
    \leq (T^\beta_t |f|)(x) \sup_{\theta \in K}|\theta| < \infty.
\end{equation}
...
% P^\beta_t |f|(x) + \Big(\sup_{x\in E}\int_{(0,1]}y^2 \pi(x,dy)\Big)~P^\beta_t |f|(x)
T^\beta_t |f|(x) + \Big(\sup_{x\in E}\int_{(0,1]}y^2 \pi(x,dy)\Big)~T^\beta_t |f|(x)
  + 2\sup_{x\in E}\int_{(1,\infty)} y \pi(x,dy)
%\\ &=: C_1 + C_2(P^\beta_t |f|)(x),
\\ &=: C_1 + C_2(T^\beta_t |f|)(x),
\end{split}
\end{equation}
...
\end{equation}
({\bf Remark: I changed $P^\beta$ to $T^\beta$ above. But I find that is it not good to make this kind of changes is not good for above equation. So, I stop to make changes. Should we use $P^\beta$?})
