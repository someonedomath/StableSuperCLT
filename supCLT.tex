%%----Versions-----------------------------
% supclt0220.tex 2019/02/20 by Jianjie
% supclt0217.tex 2019/02/14 by Renming
% supclt0214.tex 2019/02/14 by Yanxia
% supclt0211.tex 2019/02/11 by Zhenyao
% supCLT0128.tex 2019/01/28 by Renming
% supCLT0127.tex 2019/01/27 by Yanxia
% supCLT0124.tex 2019/01/24 by Jianjie
% supCLT0114.tex 2019/01/14 by Yanxia
% supCLT0107.tex 2019/01/07 by Jianjie
...
%ZS: I changed all "branching rate regime" to "branching rate regime" below.
\begin{abstract}
	%Consider a super Ornstein-Uhlenbeck processes (super-OU process) $(X_t)_{t\geq 0}$ with branching mechanism 
    Consider a $(\xi, \psi)$-superprocess $(X_t)_{t\geq 0}$ where the spatial motion $\xi$ is an Ornstein-Uhlenbeck process on $\mathbb R^d$  which has generator $L = \frac{1}{2}\sigma^2\Delta - b x \cdot \nabla$ with $\sigma, b >0$, and the
    branching mechanism $\psi$ takes the form close to
	$\psi(z)=-\alpha z +\rho z^2+ \eta z^{1+\beta}$ with $\alpha>0$, $\rho\ge 0$, $\eta>0$ and $\beta\in (0, 1)$ in some sense.
    ...
    %In the small branching rate regime $\alpha\beta< \kappa_f b(1+\beta)$, $\langle f,X_t\rangle/\|X_t\|^{\frac{1}{1+\beta}}$ converges weakly to a $(1+\beta)$-stable random variable.
    %In the critical branching rate regime  $\alpha\beta= \kappa_f b(1+\beta)$, $\langle f,X_t\rangle/(t\|X_t\|)^{\frac{1}{1+\beta}}$ converges weakly another $(1+\beta)$-stable random variable.
    %In the large branching rate regime  $\alpha\beta> \kappa_f b(1+\beta)$, we first prove that $e^{-(\alpha-\kappa(f)b)t}\langle f,X_t\rangle$ converges almost surely and in $L^{1+\gamma}$, for any $\gamma \in (0,\beta)$, and then established a central limit theorem, which says that, after proper centering and scaling, $\langle f,X_t\rangle$ again converges weakly to a $(1+\beta)$-stable random variable.
    the small branching rate regime $\alpha\beta< \kappa_f b(1+\beta)$; the critical branching rate regime $\alpha\beta = \kappa_f b(1+\beta)$; and the large branching rate regime $\alpha\beta > \kappa_f b(1+\beta)$.
\end{abstract}
...
%DELETED
	%It is known that $\alpha, \rho$ and $\pi$ are uniquely determined by the function $\psi$.
%END DELETED
...
\end{equation}	
    ...
    %followed by \cite{ChenRenSongZhang2015Strong-law, ChenRenYang2019Skeleton, EckhoffKyprianouWinkel2015Spines,KouritzinRen2014A-strong,LiuRenSong2013Strong, Wang2010An-almost}. 
    %The setups of the papers mentioned above are  different. 
    ...
    %In this paper, we focus on limit results for the super-OU process $X$.
    %The strong law of large numbers of $X$ can be stated as follows: 
    ...
    %for every Lebesgue-almost everywhere continuous bounded $f\in\mathcal B(\mathbb R^d, \mathbb R_+)$,
    ...
    %where  $H_\infty=\lim_{t\to\infty}e^{-\alpha t}\|X_t\|$, and the convergence also holds in $L^1(\mathbb P_\mu)$, see \cite[Theorem 2.13 \& Example 8.1]{ChenRenYang2019Skeleton} and \cite[Theorem 1.2 \& Example 4.1]{EckhoffKyprianouWinkel2015Spines}.
    ...
    %In this paper, we will establish some spatial central limit theorems for the super-OU process $X$ above.  
...
\]
    ...
    %for some test functions $f$.
    for some testing functions $f$.
    %It turns out that the normalization $F_t$ depends on the branching rate regimes determined by the sign of $\alpha\beta-\kappa_f b (1+\beta)$, where $\kappa_f$ is the order of $f$ in the spectral decomposition in terms of the spectrum of $L$.
...
%END ADDED
    ...
    %see \cite{KestenStigum1966Additional,KestenStigum1966A-limit} for supercritical multi-type Galton-Waston processes, 
    ...
    %All central limit results described above hold for systems with branching mechanisms with finite variance.
...
when the offspring distribution
    %belongs to the domain of a stable law of index $\alpha\in (1, 2]$, 
...
%END added
%DELETED
    %In this paper, we consider a class of super-OU processes with a general stable branching mechanism.
    %For small and critical branching rate regimes, we obtain central limit theorems which are similar to the corresponding results for branching Ornstein-Uhlenbeck processes in \cite{MarksMilos2018CLT}.
    %For large branching rate regime, we not only prove an almost sure convergence result as in \cite{MarksMilos2018CLT}, we also prove a central limit theorem type result.
%END DELETED

%DELETED    
    %For recent works on limit results (strong law of large numbers and central limit theorems) for branching diffusions and general branching Markov processes, we refer to \cite{BansayeDelmasMarsalleTran2011Limit, ChenRenSongZhang2017Law, EnglanderHarrisKyprianou2010Strong, GrubelKabluchko2016A-functional, RenSongZhang2015Central} and references there in.
%END DELETED
...
%    \cite{Le2019Long}.
%
 %   YX: I have added some parts which are related to references listed above. But I did not cite  \cite{AdamczakMilos2014U-statistics}, \cite{HartungKlimovsky2018The-phase} and \cite{Wang2018Central} since these are not closely related to our paper. I did not cite \cite{HeydeLeslie1971Improved}--\cite{IksanovKoleskoMeiners2018Stable-like} since these are about central limit theorems for Biggin's martingales.
%}
...
\end{equation}
	for some $\delta > 0$.
\end{asp}
    %{\bf Jianjie: do we need to emphasize that $\delta\in (0,1-\beta)$ such that $1+\beta+\delta<2$.}
    %ZS: It is done by Remark 1.1. 
...
\end{itemize}
%DELETED
    %Note that in the large and critical branching rate regimes, $f$ is non-trivial.
%END DELETED
...
\begin{rem}
   If $f\in \mathcal B(\mathbb R^d, \mathbb R_+)$ is non-trivial and  bounded, then $\kappa_f=0$.
    %Hence by Theorem \ref{thm: law of large number}, 
...
\]
    ...
    %In fact, to get the almost sure convergence for bounded test functions $f$, we do not need $f$ to be twice differentiable. $f$ being bounded and Lebesgue-almost every where continuous is enough for the almost sure convergence, see \cite[Theorem 2.13 \& Example 8.1]{ChenRenYang2019Skeleton} and \cite[Theorem 1.2 \& Example 4.1]{EckhoffKyprianouWinkel2015Spines}. 
...
\end{rem}
\begin{rem} 
    %For general test functions $f\in \mathcal{P}$ with $\alpha\beta>\kappa_fb(1+\beta)$, Theorem \ref{thm: law of large number} says that the correct scaling for $\langle f, X_t\rangle$ is $e^{-(\alpha-\kappa_fb)t}$.  %Theorem \ref{thm: law of large number} gives a strong law of large numbers for general test functions $f$ in the large branching rate regime, and it extends the strong laws of large numbers in  \cite{ChenRenYang2019Skeleton} and \cite{EckhoffKyprianouWinkel2015Spines} in which the first order asymptotic ($\kappa_f=0$) was identified. 
...
%DELETED 
    %For any $f \in \mathcal{P}$ with $\alpha\beta>\kappa_fb(1+\beta)$, Theorem \ref{thm: law of large number} gives an expansions of $e^{-\alpha t} \langle f, X_t\rangle$ in the following sense:
    %\[
    %e^{-\alpha t}\langle f, X_t\rangle=
    %\sum_{p\in \mathbb Z_+^d:|p|\leq\kappa_f} e^{-|p|t}\langle f, \phi_p\rangle_{\varphi} H_{\infty}^p+o(e^{-\kappa_ft}),
    %\]
    %where $\lim_{t\to\infty}e^{\kappa_ft}o(e^{-\kappa_ft})=0$ in   $L^{1+\gamma}(\mathbb{P}_{\mu})$.  
%END DELETED
...
\end{align}
    ...
    %We will show that, when $f\in \mathcal{P}$ is non-trivial, $\theta \mapsto \exp(\theta \bar m[f])$ is a characteristic function of an $(1+\beta)$-stable random variable.
...
\begin{align}
       \langle f,X_t\rangle
        %= &\sum_{k=0}^{\lfloor t \rfloor-1} \left(\mathbb P_\mu[\langle f ,X_t\rangle|\mathscr F_{t-k}] - \mathbb P_\mu[\langle f ,X_t\rangle|\mathscr F_{t-k-1}]\right)+\\
        %&\left(\mathbb P_\mu[\langle f ,X_t\rangle|\mathscr F_{{t-\lfloor t \rfloor}}] - \mathbb P_\mu[\langle f ,X_t\rangle|\mathscr F_{0}]\right)+ \langle P^\alpha_tf,X_0\rangle\\
        :=\sum_{k=0}^{\lfloor t \rfloor-1} \mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle+\mathcal I_0^{t-\lfloor t \rfloor}\langle f ,X_t\rangle +
         \langle P^\alpha_tf,X_0\rangle,
               \quad t\geq 0.
\end{align}
   %Therefore, to find the normalized limit of  $\langle f,X_t\rangle$, we need to look at each term $\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle$.
...
%DELETED
 %Note that $$
 %\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle=\langle P^\alpha_kf, X_{t-k}\rangle-\mathbb P_\mu[\langle P^\alpha_kf, X_{t-k}\rangle|\mathscr F_{t-k-1}]
 %$$
%END DELETED
...
%END ADDED
 ...
 %DELETED
 %By \eqref{lem: control of mgtrs}, we roughly have
%\begin{enumerate}
%\item If  $f$ is non-trivial and $\alpha\beta<\kappa_f b(1+\beta)$, the normalization is $F_t=\|X_t\|^{\frac{1}{1+\beta}}$, and only those $\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle$ with small $k$, normalized by $\|X_t\|^{\frac{1}{1+\beta}}$,  contribute  to the limit of $\frac{\langle f,X_t\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}$. Lemma \ref{lem: mainlemma} below gives the contribution of each term. Also, we will prove that the contributions from different time intervals are independent, and thus $\frac{\langle f,X_t\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}$ has $(1+\beta)$-stable limit as $t\to\infty$.

%\item If  $\alpha\beta=\kappa_f b(1+\beta)$, then Lemma \ref{lem: mainlemma} below says that for each $k<t$,  $\frac{\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle}{\|X_t\|^{\frac{1}{1+\beta}}}$ has a $(1+\beta)$-stable limit. But the summation of these terms will diverge. In this case, the correct normalization is $F_t=(t\|X_t\|)^{\frac{1}{1+\beta}}$.
%\item 
    %If  $\alpha\beta>\kappa_f b(1+\beta)$,  by Theorem \ref{thm: law of large number}, $e^{-(\alpha-\kappa_fb)t}\langle f, X_t\rangle$ has an almost sure limit.  
%In this case, we further consider a central limit type result for $\langle f, X_t\rangle$ when
%$f\in \mathcal{C}_l$. 
%Since each $f\in \mathcal{C}_l$ can be expressed as
%\begin{align}
    %f(x)=\sum_{p\in \mathcal{N}}a_p\phi_p(x),\quad x\in \mathbb{R}^d,
%\end{align}
%where $a_p=\langle f, \phi_p\rangle_{\varphi}$, and $\mathcal{N}$ is a finite set, we only need to consider the special case of $f=\phi$. Therefore, we only need to consider the martingale central limit result for $H^p_t, t\geq 0.$ 
%The correct normalization for each $H_{t+n-1}^p-H^p_{t+n}$ is $e^{-(\alpha-b|p|)t}e^{\frac{\alpha t}{1+\beta}}$.
%Since $\alpha\beta> |p|b(1+\beta)$, only those $H_{t+n-1}^p-H^p_{t+n}$  with small $n$, normalized by $e^{-(\alpha-b|p|)t}\|X_t\|^{\frac{1}{1+\beta}}$,  contribute  to the normalized limit of $H^p_t-H^p_\infty$. In Lemma \ref{large-central}, we will prove that for each $n$, $\frac{H^p_{t+n-1}-H^p_{t+n}}{e^{-(\alpha-|p| b)t}\|X_t\|^{\frac{1}{1+\beta}}}$ has a $(1+\beta)$-stable limit as $t\to\infty$. The limits are independent for different intervals. 
%END DELETED
...
%This paper is our first attempt to study central limit type theorems for general superprocesses with general branching mechanism. 
    ...
    %We would be interested in more general motions. 
    ...
    %Recall that we assume that the the branching mechanism satisfies Assumption \ref{asp: branching mechanism}. 
...
%DELETED
%Then the branching mechanism can be rewritten as
%\begin{equation}\label{decom-branchingMechanism}
    %\psi(z) = -\alpha z + \rho z^2+\eta z^{1+\beta} + \psi_1(z),\quad  z \in \mathbb R_+,
%\end{equation}
%where $\alpha, \eta>0$, $\rho\geq 0$, $\beta\in(0,1)$. 
%Assumption \ref{asp: branching mechanism} is equivalent to that the branching mechanism is given in the form of \eqref{decom-branchingMechanism} with $\psi_1=\psi^+_1-\psi^-_1$, where $\psi^+_1=\int_{(0,\infty)}(e^{-yz}-1+yz)\nu^+(dy)$, and $\psi^-_1=\int_{(0,\infty)}(e^{-yz}-1+yz)\nu^-(dy)$ with $\nu^+$ and $\nu^+$ being measures on $(0,\infty)$ such that
%$$\int_{(0,\infty)}(y^{1+\beta +\delta}\wedge y^2)(\nu^++\nu^-)(dy)<\infty$$ for sufficient small $\delta>0$. 
%Basically, our  Assumption \ref{asp: branching mechanism} says that the branching mechanism $\psi$ is $-\alpha z + \rho z^2+\eta z^{1+\beta}$ plus a a small perturbation $\psi_1$.
    It would be interesting to consider more general branching mechanism.

%DELETED
%Our results show that he behavior of $\psi(z)+\alpha z$ near $z= 0$ plays the dominating role. 
%The limit and the normalization only depend on $\eta z^{1+\beta}$. It is interesting to consider more general branching mechanism.
%END DELETED 

%We wend this section with some examples of branching mechanisms satisfying Assumptions \ref{asp: Greys condition} and \ref{asp: branching mechanism}.
    ...
    %If $\varphi$ is a complete Bernstein function which is regularly varying at 0 with index
    If $h$ is a complete Bernstein function which is regularly varying at 0 with index
$\beta_1\in (\beta, 1)$, then 
$$
%\psi(z):=-\alpha z + \rho z^2+\eta z^{1+\beta}+z\varphi(z), \qquad z>0
...
\item[(7)]
       %For any $\delta > 0$ small enough, we have that $(K_t)_{0\leq t\leq 1}$ is uniformly $(\theta^{2+\beta}\vee \theta^{1+\beta+\delta})$-controllable on $\mathcal P$.
...
\]
    ...
    %(7). Since $K_t = (Z'_t-Z_t)+Z''_t$, the desired result follows from parts (5)-- (6) of this lemma  and Lemma \ref{lem: property of controllable operators}.(3).
...
\[
    \bar{m}_t[f]
    %:= \eta \int_{t}^{t+1} e^{\alpha s}~ds \int_{\mathbb R^d}\big(iI_sf(x)\big)^{1+\beta} \varphi(x)~dx, \quad t\geq 0.
...
%DELETED
    %by \cite[(2.24)]{RenSongZhang2014Central}, 
%END DELETED
...
\label{ineq: control of partial derivative of Itg}
    ...
    \quad  t\geq 0, x\in \mathbb R^d.
\end{align}
	%In fact, if we put $h_1(x)=\Big|\sum_{p\in \mathcal N}\langle g, \phi_p\rangle_\varphi\frac{\partial}{\partial x_i}\phi_p(x)\Big| \in \mathcal P$, then
...
%END NEW ADDED
%DELETED
    %According to \eqref{ineq: control of Itg} and \eqref{ineq: control of partial derivative of Itg}, we easily get there exists $h_2 \in \mathcal P$ such that for all $t\geq 0$,
%\begin{align}
    %&\Big|\frac{\partial}{\partial x_i}\bar{g}_t(x)\Big| 
    %= e^{(\alpha-Kb)(1+\beta)t}\Big|\frac{\partial}{\partial x_i}Z_1(-I_tg)\Big| \\
    %& = (1+\beta) \eta e^{(\alpha-Kb)(1+\beta)t}\Big |\int_0^1 P_{1-s}^{\alpha}\Big((iP_s^{\alpha} I_tg)^{\beta}\cdot i P_s^{\alpha}(\frac{\partial}{\partial x_i}I_tg(x))\Big)~ds\Big| \\
    %& \leq (1+\beta) \eta  e^{(\alpha-Kb)(1+\beta)t}
    %\int_0^1 P_{1-s}^{\alpha}\Big((P_s^{\alpha} |I_tg|)^{\beta}\cdot P_s^{\alpha}(\big|\frac{\partial}{\partial x_i}I_tg(x)\big|)\Big)~ds \\
    %& \leq h_2(x).
%\end{align}
%END DELETED
%Therefore, there exists $h_3 \in \mathcal P$, such that
Therefore, there exists constants $c,n>0$ such that 
\begin{align}\label{ineq: control of sup gt}
    %\sup |\nabla \bar g_t (z)| \leq h_3(x)+h_3(y),
    %\quad t\geq 0,~ x,y \in \mathbb R^d,
    |\nabla \bar g_t (z)| 
    \leq c(1+|z|)^n,
...
%END NEW ADDED
%Deleted
%where supermum is taken over $z$ in the interval connecting $xe^{-bs}+y\sqrt{1-e^{-2bs}}$ with $y$.
%By the fact $\langle \bar{g}_t,\varphi\rangle = 0$, we may write
%\begin{align}
    %&|P_s\bar{g}_t| = \Big|\int_{\mathbb R^d} \big(\bar {g}_t(xe^{-bs}+y\sqrt{1-e^{-2bs}})-\bar {g}_t (y)\big) \varphi(y)~dy \Big| \\
    %& \leq \int_{\mathbb R^d} \sup |\bigtriangledown \bar g_t (z)|\Big|xe^{-bs}+y\sqrt{1-e^{-2bs}}-y \Big| \varphi(y)~dy \\
    %& \leq \int_{\mathbb R^d} ( h_3(x)+h_3(y))|\Big|xe^{-bs}+y\sqrt{1-e^{-2bs}}-y \Big| \varphi(y)~dy.
%\end{align}
 %We get the conclusion by the fact that there exists $h_4 \in \mathcal P$ such that
%\begin{align}
    %\Big|xe^{-bs}+y\sqrt{1-e^{-2bs}}-y \Big|\leq e^{-bs}(h_4(x)+h_4(y)),
    %\quad s\geq 0,~x,y \in \mathbb R^d.
%\end{align}
\end{proof}
...
%\subsection{The large rate regime}
...
\begin{lem}\label{lem: control of wt}
    %For all $\gamma\in (0,\beta)$, $p\in \mathbb{Z}_+^d$ and $\mu\in \mathcal M_c(\mathbb R^d)$, there exists $C> 0$ such that for all $0\leq s<t$,
...
\begin{align}
    ...
    %\leq C_1e^{\alpha t+(t-s)(\alpha\gamma-(1+\gamma)|p|b\gamma}.
...
\end{proof}
    ...
    %{\it Proof of Theorem \ref{thm: law of large number}.}\quad
...
\end{align}
    ...
    %In fact, it follows from \eqref{eq: complex FKPP-equation with FK-transform}, the definitions of $U_1$, $Z'''_1$ and $Z_1$, that for all $k\geq 0, t\geq k+1$,
...
\end{align}
    %Therefore, it follows from \eqref{eq: prevJ2} that there exists constant $C_3>0$ such that for all $k\geq 0$, $t\geq k+1$ and $\epsilon> 0$,
...
\label{eq: Y in D}
    ...
    %\\&= C_6 e^{(\frac{\alpha}{1+\gamma}-\frac{\alpha}{1+\beta})(t-k)}e^{-(\eta_0 - 3\eta_1)(t-k-1)},
...
\begin{align}
    ...
    %= C_8 e^{-\delta t}\sum_{k=0}^{\lfloor t-\ln t \rfloor} e^{\delta_6 k}
    = C_8 e^{-\delta_6 t}\sum_{k=0}^{\lfloor t-\ln t \rfloor} e^{\delta_6 k}
    %\\&\leq C_9 e^{-\delta t}e^{\delta_6 (t-\ln t)}
...
\begin{equation}
    %\mathbb{\tilde{P}}_{\mu}(\mathcal{E}^c_t)\leq \frac{1}{\mathbb{P}_{\mu}(D^c)}\mathbb{P}_{\mu}(0<e^{-\alpha t}\|X_t\|<t^{-1/2})\leq C_{10}( t^{-\delta_7}+e^{-\delta_7 t}), \quad t\geq0.
...
\label{eq: condition for supercritical}
	&-(\frac{\alpha}{1+\gamma}-\frac{\alpha}{1+\beta})
    = \alpha(1-\frac{1}{1+\gamma}) - \alpha(1-\frac{1}{1+\beta})
    %= \alpha(1-\frac{\gamma}{1+\gamma}) - \alpha(1-\frac{\beta}{1+\beta})
    \\&> \frac{\gamma \alpha}{1+\gamma} - k_f b
    =\frac{\alpha \gamma-\kappa_f(1+\gamma)b}{1+\gamma}.
\end{align}
...
\end{proof}
%\subsection{Proof of Theorem \ref{thm: large clt}}
...
\end{align}
    ...
    %Recall that $\{H^p_t, \mathscr{F}_t\}_{t\geq 0}, p\in \mathbb{Z}_+^d,$ are defined before Lemma \ref{lemma26}.
...
\end{lem}
\begin{proof}
    %We fix $n\ge 1$ and $\mu\in\mathcal{M}_c(\mathbb{R}^d)$.
...
\end{align}
    ...
    %$A_t(\epsilon):=\{\|X_t\|\geq e^{(\alpha-\epsilon)t}\}$.
...
\[
	|e^{-x} - e^{-y}| \leq |x-y|,\quad x,y \in \mathbb C_+,
\]
    %we have for all $n\geq 1, t\geq 0, \epsilon> 0$,
...
\label{eq: inequality that will used later1}
    ...
    %\\&\leq \mathbb{P}_{\mu}\Big[\big|\langle (K_1+Z_1)(-\sum_{p\in \mathcal{N}}
...
\end{align}
    ...
    Step 2.
    %We will show that for $\epsilon>0$ small enough, there exist constants $C_1,\delta_1>0$ such that for all $n\geq 1, \, t\geq 0$,
...
\end{align}
	%Let $\delta_0$ be the constant in Lemma \ref{lem: upper bound for usgx}.(7) and $R$ be the $(\theta^{1+\beta}\vee \theta^{1+\beta+\delta_0})$-controller. 
...
\end{align}
    ...
    %Thus for all $n\geq 1,  t\geq 0, \epsilon > 0$,
...
%END ADDED
    ...
    Step 3.
	%We will show that for each $\epsilon$ small enough, there exist $C_2,\delta_2>0$ such that for all $n\geq 1, t\geq 0$,
...
\begin{align}
    ...
    %\leq C_3e^{\frac{\alpha \gamma}{1+\gamma}(t+n-1)},\quad t\geq0, n\geq 1.
...
%END ADDED    
	...
	%Step 4. Using an argument similar to that used in Step 4 of the proof of  Lemma \ref{lem: mainlemma}, we can show that for any $\epsilon\in (0,  \alpha)$ there exist $C_4,\delta_3>0$ such that
...
\[
    \tilde{a}_n
    %:=\mathbb{\tilde{P}}_{\mu}\Big(\prod_{l=0}^{n}\exp\left(i\theta\bar{\gamma}_{t,n}\right)\prod_{l=n+1}^{m}\exp(\bar{m}_l[\theta g])\Big),
    :=\mathbb{\tilde{P}}_{\mu}\Big[\prod_{l=0}^{n}\exp(i\theta\bar{\gamma}_{t,l})\Big] \times \prod_{l=n+1}^{m}\exp(\bar{m}_l[\theta g]),
\] 
    %where by convention the product is $1$ for $n=0$. Then we get for each $n \ge 1$,
...
\begin{align}
    ...
    %\quad t\geq 0, n\geq 1, \theta\in \mathbb{R}.
    \quad t\geq 0, n\geq 1.
\end{align}
    Let $\gamma \in (0,\beta)$ be close enough to $\beta$ such that
\[
...
\end{align}
    ...
    Step 2.
    %We will show that for each $\mu \in \mathcal{M}_c(\mathbb{R}^d)$, there exist $C_2,\delta_2 > 0$ such that for all $n\geq 1$ and $t\geq 0$,
...
\label{ineq: control of Kkt1}
     ...
     \quad t \geq 0, n\geq 1.
\end{align}
    %According to Lemma \ref{lem: control of wt}, there exist constants $C_4,C_5>0$ such that  for all $t\geq 0$ and $n\geq 1$,
...
\begin{align}\label{ineq:used next 2}
 ...
 %DELETED   
    %\\&\leq C_5|\theta|\exp\Big\{\Big(\frac{\alpha}{1+\gamma}-\frac{\alpha}{1+\beta}-(\eta_0-3\eta_1)\Big)t\Big\}.
%END DELETED
\end{align}
...
%END ADDED
    %Recall that $A_t(\epsilon)=\{\|X_t\|>\|\mu\|e^{(\alpha-\epsilon) t}\}$. 
...
\end{align}
%DELETED
    %Fix $\epsilon>0$ and $\gamma\in(0,\beta)$ with $\alpha\gamma>Kb(1+\gamma)$.  
%END DELETED
...
\end{align}
    %According to Lemma \ref{lem: independency for large rate}, there exist $C_1,\delta_1>0$ such that for all $t\geq 0,\theta\in\mathbb{R}$,
...
\end{align}
    %According to Lemma \ref{lem: lemma04}, there exists $C_2>0$ such that for all $t\geq 0,\theta\in\mathbb{R}$,
...
\begin{align}\label{ineq: last2}
 ...
 \end{align}
    %According to Lemma \ref{lem: lemma05}, there exist $C_3,\delta_2,\delta_3>0$ such that for all $t\geq 0,\theta\in\mathbb{R}$,
...
\[
   h(r) \leq e^{t\|\rho_1\|_\infty}\|\rho_2\|_\infty \int_r^t h(s)ds,
    \quad r\le t.
\]
...
\label{eq: upper bound of psi'(v)}
    ...
    %\\ &\leq \|\rho_1\|_\infty + 2\rho(x)_2\mathbb N_x[\langle W_t, |f|\rangle]+ \int_{(0,\infty)} y\mathbf P_{y\delta_x}[2\wedge \langle X_t, |f|\rangle] \pi(x,dy)
...
Z.-Q. Chen, Y.-X. Ren, and H.~Wang, \emph{An almost sure scaling limit theorem
 ...
 %\bibitem{ChenRenSongZhang2017Law} Z.-Q. Chen, Y.-X. Ren,  and T. Yang, \emph{Law of large numbers for branching symmetric Hunt processes with measure-valued branching rates},  J. Theoret. Probab. \text{30} (2017), no.~3, 898--931.
