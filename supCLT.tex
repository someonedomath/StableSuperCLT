...
\end{equation}	
    ...
    \deleted {Note that $(\|X_t\|)_{t\geq 0}$ is a continuous-state branching process with branching mechanism $\psi$.}
...
\end{equation}
    ...
    According to \cite[Fact 1.2]{MM}, for any $f \in \mathcal{P}$, there exists $\deleted{R} \in \mathcal{P}$ such that,
    
    {\bf Jianjie: we repeatedly use $R$ as both a function and a operator.}
\begin{equation}
...
\end{equation}
	For each $f\in \mathcal {P}$, define an operator $Q$ by $Qf(x):=\sup_{t\geq 0}e^{\kappa_f bt} |T_tf(x)|$.
...
\end{equation}
    ...
    For each $f\in \mathscr B(\mathbb R^d, \mathbb C_+)$ and $x\in \mathbb R^d$, define $\Psi_0f(x) := \psi_0(\deleted {x,}f(x))$ and $\Psi'_0 f(x) := \psi'_0(\deleted {x,}f(x))$.
...
\[
    ...
    \leq \frac{ 1}{\lambda^{1+\beta}} e^{(1+\beta)(t-s)(\alpha- \kappa_gb)+ \alpha r} \langle SQg,\mu\rangle.
\]
\end{lem}
\begin{proof}
...
\begin{align}
    ...
    \\& \leq c^{1+\gamma} + e^{(1+\beta)(t-s)(\alpha- \kappa_gb) + \alpha r} \langle SQg,\mu\rangle  (1+\gamma)\int_c^\infty \frac{1}{\lambda^{1+(\beta-\gamma)}}d\lambda
    \\&\leq c^{1+\gamma} e^{\alpha r} + e^{(1+\beta)(t-s)(\alpha- \kappa_gb) + \alpha r} \langle SQg,\mu\rangle   \frac{1+\gamma}{\beta - \gamma} \frac{1}{c^{\beta - \gamma}}.
\end{align}
...
\end{align}
\added{{\bf Zhenyao: we should show that it is well defined.}}
...
\begin{proof}
    ...
    Using  Lemma \ref{lemma24} and the fact $\kappa_{\phi_p}=|p|$, we get that, there exists a constant $C_{\gamma, \mu, p}>0$ (which depends on the choice of $\gamma$, $\mu$ and $p$)  such that
 $$
...
$\mathbb{P}_{\mu}$-a.s and in $L^{1+\gamma}(\mathbb{P}_{\mu})$. So $W_{\infty}$ is non-degenerate and the $(1+\gamma)$-th moment is finite. Moreover,  we have $\mathbb{P}_{\mu}(W_{\infty})=\|\mu\|$ and $\{W_{\infty}=0\}=D,$ $\mathbb{P}_{\mu}$-a.s.
...
\added{Note that $(\|X_t\|)_{t\geq 0}$ is a continuous-state branching process with branching mechanism $\psi$. According to \cite[Example 3.1]{Li2011Measure-valued}, we have
...
Recall $\mathcal{A}_t(\epsilon)=\{\|X_t\|\in\|\mu\|(e^{(\alpha-\epsilon)t},e^{(\alpha+\epsilon)t})\},~\epsilon,t>0$, then for any $\epsilon>0$, we have
\begin{align}
...
\begin{proof}
    ...
    For any $\omega \in \{H_{\infty}>0\}\cap\{\lim_{t\rightarrow \infty} H_t=H_{\infty}\}$, there exists $\delta>0$ such that $H_{\infty}(\omega)\in(\delta,\frac{1}{\delta})$ and there exists $S_1>0$ such that for any $t\geq S_1$, we have $\frac{\delta}{2}\leq H_t(\omega)\leq 2\delta,$. Taking $S_2>0$ such that for any $t>S_2$, $\|\mu\|e^{-\epsilon t}<\frac{\delta}{2}$ and $\|\mu\|e^{-\epsilon t}>2\delta$, then $\omega\in \mathcal{A}_t(\epsilon),~t>S:=\max\{S_1,S_2\}$ implies $\omega \in \liminf_{t\rightarrow \infty}\mathcal{A}_t(\epsilon):=\cup_{S>0}\cap_{t\geq S}\mathcal{A}_t(\epsilon)$. 
    
    According to \cite{LSR}, $D^c=\{H_{\infty}>0\}~ \mathbb{P}_{\mu}$-a.s, combining with the fact $\lim_{t\rightarrow \infty}H_t=H_{\infty}~\mathbb{P}_{\mu}$-a.s, we get the result.
\end{proof}
}
\begin{lem}\label{lemma33}

 Let $f\in \mathcal{P}$, assume that $\alpha\beta\leq \kappa_fb(1+\beta)$, then for any  $k\in\mathbb{N}$ and $\mu \in \mathcal{M}_c(\mathbb{R}^d)$, under $\mathbb{P}_{\mu}(\cdot | D ^c)$, we have
 \begin{align}
      \frac{I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{F_k(X_{t-k-1})}\xrightarrow{d}\zeta_k, \quad t\rightarrow \infty, \label{limitdistribution1}
 \end{align}
 where $\zeta_k$ is a $(1+\beta)$-stable random variable with characteristic function
 $$\mathbb{E}e^{i\theta\zeta_k}=\exp\left[e^{\alpha}m_k[\theta f]\right].$$
 \end{lem}

 \begin{proof}
For each $\theta \in \mathbb{R}$, let $\tilde{\theta}_{t,k}=\frac{\theta}{F_k(X_{t-k-1})}$. For any $k\in\mathbb{N}$ and $t>k+1$, we have
         \begin{align}
        &\mathbb{P}_{\mu}\big[\exp(i\theta\frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{F_k(X_{t-k-1})})\big|\mathscr{F}_{t-k-1}\big]\\
     ...
     \begin{align}
        \Big|\mathbb{P}_{\mu}\Big[\big(\exp(i\theta \frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{F_k(X_{t-k-1})})-\exp(e^{\alpha}m_k[\theta f])\big)\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\Big]\Big|\xrightarrow{t\rightarrow\infty} 0.
    \end{align}
      According to \eqref{condition mean},
    \begin{align}
        &\Big|\mathbb{P}_{\mu}\Big[\big(\exp(i\theta \frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{F_k(X_{t-k-1})})-\exp(e^{\alpha}m_k[\theta f])\big)\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\Big]\Big|\\
        &=\Big|\mathbb{P}_{\mu}\Big[\mathbb{P}_{\mu}\Big(\big(\exp(i\theta \frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{F_k(X_{t-k-1})})-\exp(e^{\alpha}m_k[\theta f])\big)\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\Big|\mathscr{F}_{t-k-1}\Big)\Big]\Big|\\
        ...
        \label{ineqinlem31}
        &+\mathbb{P}_{\mu}\big[\big|\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]\big|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\big].
    \end{align}
Step 1.  We will show that for each $\mu \in\mathcal{M}_c(\mathbb{R}^d)$, there exists $C>0$, such that for any $\theta\in \mathbb{R}$, $k\in\mathbb{N}$ and $t>k+1$,
    \begin{align}
    \label{lemma31q}
        \mathbb{P}_{\mu}\big[\big|\langle  U_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)-\tilde{U}_1(\tilde{\theta}_{t,k}T_k^{\alpha}f), X_{t-k-1}\rangle\big|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\big]\leq C|\theta|^{1+2\beta}e^{-\frac{\alpha\beta}{2(1+\beta)}(t-k-1)}.
    \end{align}
Let $(R,S)$ be the $(1+2\beta)$-control-pair in Lemma \ref{lem: upper bound for usgx} (6). For any $\theta\in\mathbb{R}$, $k \in \mathbb N$ and $t>k+1$, we have
\begin{align}
   & \big|U_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)-\tilde{U}_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)\big|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\leq R\big(|\tilde{\theta}_{t,k}T_k^{\alpha}f|\big)\leq R \Big(\frac{|\theta|e^{(\alpha-\kappa(f)b)k}}{(e^{\alpha k}\|\mu\|e^{(\alpha-\epsilon)(t-k-1)})^\frac{1}{1+\beta}}Qf\Big)
   \\& \leq S \Big(\frac{|\theta|e^{(\alpha-\kappa(f)b)k}}{(e^{\alpha k}\|\mu\|e^{(\alpha-\epsilon)(t-k-1)})^\frac{1}{1+\beta}}Qf\Big) 
   \leq \Big(\frac{|\theta|e^{(\alpha-\kappa(f)b)k}}{(e^{\alpha k}\|\mu\|e^{(\alpha-\epsilon)(t-k-1)})^\frac{1}{1+\beta}}\Big)^{1+2\beta}SQf
   \\&=\frac{1}{\|\mu\|^{(1+2\beta)/(1+\beta)}}|\theta|^{1+2\beta}e^{-\frac{\alpha(1+2\beta)(t-k-1)}{1+\beta}}e^{\frac{\epsilon_0(1+2\beta)(t-k-1)}{1+\beta}}SQf.
\end{align}
Thus,
\begin{align}
     &\mathbb{P}_{\mu}\big[\big|\langle  U_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)-\tilde{U}_1(\tilde{\theta}_{t,k}T_k^{\alpha}f), X_{t-k-1}\rangle\big|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\big]\\
     &\leq \frac{1}{\|\mu\|^{(1+2\beta)/(1+\beta)}}|\theta|^{1+2\beta}e^{-\frac{\alpha(1+2\beta)(t-k-1)}{1+\beta}}e^{\frac{\epsilon_0(1+2\beta)(t-k-1)}{1+\beta}}\mathbb{P}_{\mu}[\langle SQf,X_{t-k-1}\rangle]\\
...
\end{align}
Step 2, for any $\mu \in \mathcal{M}_c(\mathbb{R}^d)$ and $\theta\in \mathbb{R}$, we will show that there exists $C>0$ such that
\begin{align}
\label{eq:31step3}
    &\mathbb{P}_{\mu}\big[\big|\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]\big|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\big]\\
    &\leq C|\theta|^{1+\beta} e^{-\frac{p}{2}(t-k-1)},\quad k\in \mathbb{N}, t\geq k+1.
\end{align}
    Notice that $\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]=\theta^{1+\beta}e^{-\alpha k}(\frac{\langle Z_{f_k},X_{t-k-1}\rangle}{\|X_{t-k-1}\|} \rangle-\langle Z_{f_k},\varphi\rangle)$. Therefore,
    
\begin{align}
        &\mathbb{P}_{\mu}\left[\left|\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\right]\\
    &=|\theta|^{1+\beta}e^{-\alpha k}\mathbb{P}_{\mu}\left[\left|\frac{\langle Z_1(T^{\alpha}_k f),X_{t-k-1}\rangle}{\|X_{t-k-1}\|}-\langle  Z_1(T^{\alpha}_k f)Z_{f_k},\varphi\rangle\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\right]\nonumber\\
...
\end{align}
where $h_k(x):= Z_1(T^{\alpha}_k f)(x)-\langle  Z_1(T^{\alpha}_k f),\varphi\rangle,~x\in\mathbb{R}^d$. According to \cite[Lemma 2.8]{MM}, there exists $R_7\in \mathcal{P}$ such that for any $k\in\mathbb{N}$, $x\in \mathbb{R}^d$ and $s\geq 0$,
 \begin{align}
 \label{eq:31step2}
     |T_s h_k(x)|\leq e^{-bs}e^{(\alpha-\kappa_fb)(1+\beta)k}R_7(x).
 \end{align}
\deleted{
...
{\bf Jianjie: I think we should change the condition of Lemma \ref{lem: control moment} }
...
Let $f\in \mathcal{P}$ and $\Theta>0$, assume that $\alpha\beta\leq\kappa(f)b(1+\beta)$. There exists $C,\delta>0$ such that for any $n \in \{0,...,\lfloor t \rfloor\}$ and $(\theta_0,...,\theta_n)\in \mathbb{R}^{n+1}$ satisfying $|\theta_i|\leq \Theta$, we have
\begin{align}
\label{32corollary}
...
\end{cor}
\begin{proof}
    Denote $\gamma_{t,k}=\frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{(e^{\alpha k}\|X_{t-k-1}\|)^\frac{1}{1+\beta}} $, we define
    $$\varphi^k_t(\theta_0,...,\theta_n):=\prod_{l=0}^{k}\exp\left(e^{\alpha}m_l[\theta_lf]\right)\mathbb{\tilde{P}}_{\mu}\left(\prod_{l=k+1}^{n}\exp\left(i\theta_l\gamma_{t,l}\right)\right)$$
    for $k\in\{-1,...,n\}$, if $k=-1$, the product is one. Then Conditioning with $\mathscr{F}_{t-k-1}$ we get
    \begin{align*}
        \varphi^{k-1}_t(\theta_0,...,\theta_n)-&\varphi^{k}_t(\theta_0,...,\theta_n)=\prod_{l=0}^{k-1}\exp\left(e^{\alpha}m_l[\theta_l f]\right)\\
        &\mathbb{\tilde{P}}_{\mu}\left[\left(\mathbb{\tilde{P}}_{\mu}\left(e^{i\theta_k \gamma_{t,k}}|\mathscr{F}_{t-k-1}\right)-e^{e^{\alpha}m_k[\theta_kf]}\right)\prod_{l=k+1}^n\exp(i\theta_l \gamma_{t,l})\right]
    \end{align*}
    Hence,
    \begin{align*}
        \left|\varphi^{k-1}_t(\theta_0,...,\theta_n)-\varphi^{k}_t(\theta_0,...,\theta_n)\right| \leq \mathbb{\tilde{P}}_{\mu}\left|\mathbb{\tilde{P}}_{\mu}\left(e^{i\theta_k \gamma_{t,k}}|\mathscr{F}_{t-k-1}\right)-e^{e^{\alpha}m_k[\theta_kf]}\right|.
    \end{align*}
    Step 1, we will prove that there exists $C,\delta>0$ such that for any $|\theta|<\Theta$ and $k\in \{0,...,\lfloor t\rfloor\}$,
    \begin{align}
    \label{eq:32step1}
        \left|\mathbb{\tilde{P}}_{\mu}\left(e^{i\theta \gamma_{t,k}}|\mathscr{F}_{t-k-1}\right)-e^{e^{\alpha}m_k[\theta f]}\right|\leq C e^{-\delta(t-k-1)}.
    \end{align}
 
 By \eqref{lemma31q} and \eqref{eq:31step3}, we easily get, if $|\theta|\leq\Theta$ and $k\in\{0,...,\lfloor t\rfloor\}$,
    \begin{align*}
        &\mathbb{\tilde{P}}_{\mu}\left[\left|\langle  U_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)-\tilde{U}_1(\tilde{\theta}_{t,k}T_k^{\alpha}f), X_{t-k-1}\rangle\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\right]\leq \frac{C}{\mathbb{P}_{\mu}(D^c)}|\Theta|^{1+2\beta} e^{-\frac{\alpha\beta}{2(1+\beta)}(t-k-1)}\\
        &\mathbb{\tilde{P}}_{\mu}\left[\left|\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\right]\leq \frac{C}{\mathbb{P}_{\mu}(D^c)}|\Theta|^{1+\beta} e^{-\frac{p}{2}(t-k-1)}.
    \end{align*}
 Define
\begin{align*}
    \mathcal{B}_{t-k-1}:=\left\{\left|\langle U_1(\tilde{\theta}_{t,k}T_k^{\alpha}f)-\tilde{U}_1(\tilde{\theta}_{t,k}T_k^{\alpha}f), X_{t-k-1}\rangle\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\leq\frac{C}{\mathbb{P}_{\mu}(D^c)}|\Theta|^{1+2\beta} e^{-\frac{\alpha\beta}{4(1+\beta)}(t-k-1)}\right\}.
\end{align*}
...
\begin{align*}
    \mathcal{C}_{t-k-1}:=\left\{\left|\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]\right|\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\leq \frac{C}{\mathbb{P}_{\mu}(D^c)}|\Theta|^{1+\beta} e^{-\frac{p}{4}(t-k-1)}\right\},
\end{align*}
...
\begin{align}
    ...
    \\&\leq \sum_{k=0}^n C e^{-\delta_3(t-k-1)}.
\end{align}
