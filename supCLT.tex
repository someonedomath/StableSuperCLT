...
\begin{align}
    ...
    \begin{align}
        %&\Big|\mathbb{P}_{\mu}\Big[\big(\exp(i\theta \frac {I_{t-k-1}^{t-k}[\langle f ,X_t\rangle]}{(e^{\alpha %k}\|X_{t-k-1}\|)^{\frac{1}{1+\beta}}})-\exp(e^{\alpha}m_k[\theta f])\big)\mathbf{1}_{\mathcal{A}_{t-k-1}(\epsilon_0)}\Big]\Big|\\
...
\begin{proof}
    ...
    \begin{align*}
        ...
        %\\&=\prod_{l=0}^{k-1}\exp(e^{\alpha}m_l[\theta_l %f])\mathbb{\tilde{P}}_{\mu}\Big[\prod_{l=k}^{n}\exp(i\theta_{l}\gamma_{t,l})-\exp(e^{\alpha}m_k[\theta_k %f])\prod_{l=k+1}^n\exp(i\theta_{l}\gamma_{t,l})\Big]
