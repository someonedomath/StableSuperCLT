...
\added{Write $\mathbb{\tilde{P}}_{\mu}=\mathbb{P}_{\mu}(\cdot|D^c)$.}
...
\begin{lem}\label{lemma32}
    \deleted{Recall} \added{Write} $\mathcal{A}_t(\epsilon)=\{\|X_t\|\in\|\mu\|(e^{(\alpha-\epsilon)t},e^{(\alpha+\epsilon)t})\},$ for each $\epsilon,t>0$. 
...
Then for any  \deleted{$k\in\mathbb{N}$} \added{$k\geq 0$} and $\mu \in \mathcal{M}_c(\mathbb{R}^d)$, under $\mathbb{P}_{\mu}(\cdot | D ^c)$, we have
 \begin{align}
 ...
 \begin{proof}
For each $\theta \in \mathbb{R}$, let $\tilde{\theta}_{t,k}=\frac{\theta}{F_k(X_{t-k-1})}$. For any \deleted{$k\in\mathbb{N}$} \added{$k\geq 0$} and $t>k+1$, we have
{\bf Zhenyao: Could you change $k\in \mathbb N$ to $k>0$ universilly?}
         \begin{align}
    ...
    \end{align}
    {\bf Zhenyao: Give more detail about the above assertion. For example, We used the fact that $\exp(e^{\alpha}m_k[\theta f])$ is bounded.}
    ...
    \end{align}
{\bf Zhenyao: I think the last inequality for the above assertion needs more details. It seems that it use the result that $|e^x-e^y|\leq (|e^x|\wedge |e^y|)|x-y|$ for $x,y\in \mathbb C$.  }

Step 1.  We will show that for each $\mu \in\mathcal{M}_c(\mathbb{R}^d)$, there exists $C>0$, such that for any $\theta\in \mathbb{R}$, \added{$k\geq 0$} and $t>k+1$,
    \begin{align}
    \label{lemma31q}
    ...
    \end{align}
    \added{In fact,} let $(R,S)$ be the $(1+2\beta)$-control-pair in Lemma \ref{lem: upper bound for usgx} (6). For any $\theta\in\mathbb{R}$, $k \in \mathbb N$ and $t>k+1$, we have
\begin{align}
   ...
   \\&\leq R \Big(\frac{|\theta|e^{(\alpha-\kappa(f)b)k}}{(e^{\alpha k}\|\mu\|e^{(\alpha-\epsilon)(t-k-1)})^\frac{1}{1+\beta}}Qf\Big)
...
\end{align}
{\bf Zhenyao: I can't see the last step. It seems that you used the definition of $\epsilon_0$. Could you please explain more? Espicially, how $\kappa_f$ vanished, and how this is an equality?}
...
\end{align}
{\bf Zhenyao: I can't see the last step either. It seems that you used the definition of $\epsilon_0$. Could you please explain more?}
...
\end{align}
    Notice that $\langle Z_1(\tilde{\theta}_{t,k}T_k^{\alpha}f),X_{t-k-1}\rangle-e^{\alpha}m_k[\theta f]=\theta^{1+\beta}e^{-\alpha k}(\frac{\langle Z_{f_k},X_{t-k-1}\rangle}{\|X_{t-k-1}\|} \rangle-\langle Z_{f_k},\varphi\rangle)$. 

    {\bf Zhenyao: The above assertion is not right. $\theta^\beta$ has no meaning if $\theta < 0$. And we don't use notation $Z_f$ anymore. It seems to me that $m_t[\theta f] = |\theta|^{1+\beta}m_t[f\operatorname{sgn}\theta]$.}
...
We want to emphasize that, on the case $\alpha\gamma_0=(1+\gamma_0)b$, the second equation holds when $t$ is large enough. 
Thus
\begin{align}
