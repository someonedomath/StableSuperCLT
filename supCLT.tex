%%----Versions-----------------------------
%supCLT-2.tex 2019/03/05 by Renming
%supCLT.tex 2019/03/04 by Zhenyao
...
Let $\psi$ be a branching mechanism which is close to a function of the form
$\widetilde{\psi}(z)=-\alpha z +\rho z^2+ \eta z^{1+\beta}$ with $\alpha>0$, $\rho\ge 0$, $\eta>0$ and $\beta\in (0, 1)$, in some sense. In this paper, we study asymptotic behaviors of 
$(\xi, \psi)$-superprocesses $(X_t)_{t\geq 0}$.
...
%    $\phi_0$, $\varphi$ are functions which will be introduced in  \eqref{eigenfunction} and \eqref{invariantdensity} below.
...
%    It turns out that there are three regimes of different behavior of this CLT 
    ...
    These two papers made connections between the CLT and 
    %what they call branching rate regimes.
    the branching rate regimes
...
%    They established some central limit theorems in small and critical branching rate regimes, 
   They established some central limit theorems in the small and critical branching rate regimes, 
...
%    first one in the literature on establishing CLT 
...
\begin{equation}
    ...
    p \in \mathbb{Z}_+^{d}.
\end{equation}
%       It is known that the transition semigroup $(P_t)_{t\geq 0}$ of the OU process $\xi$ 
...
%    The fluctuation result associated to the law of large numbers are 
...
\end{align}
    We will show in Lemma \ref{lem: def of all m} that $\bar{m}[f]$ is well-defined for each $f\in \mathcal C_l$; and in Lemma \ref{prop: alpha stable rv} that that, when $f\in \mathcal{P}$ is non-trivial, 
%    $\theta \mapsto \exp(\theta \bar m[f])$ is a characteristic 
...
%   $\theta \mapsto \exp(\theta \widetilde m[f])$ is a characteristic function of an 
   $\theta \mapsto \exp( \widetilde m[\theta f])$ is a characteristic function of a
    $(1+\beta)$-stable random variable.
\begin{thm}
...
\end{equation}
	...
	We will prove  in Lemma \ref{prop: alpha stable rv} that 
%	$\theta \mapsto \exp(\theta m[f])$ is a characteristic function of an
	$\theta \mapsto \exp( m[\theta f])$ is a characteristic function of a
	 $(1+\beta)$-stable random variable.
\begin{thm}
\label{thm: small clt}
       If $f\in\mathcal{P}$ is non-trivial and satisfies $\alpha\beta<\kappa_f b(1+\beta)$, 
...
\end{align}
   ...
   %each term $\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle$.
...
\]   
    ...
    Lemma \ref{lem: mainlemma} and Corollary \ref{cor: indepedent of the limit zeta for critical and small} below imply that if $\alpha\beta \leq \kappa_f b(1+\beta) $ then
\[
...
\]
    ...
    In explanation above, we have used the facts that $(\zeta_k)_{k\in \mathbb N}$, $\zeta$ are all well defined,
\begin{equation}
...
\]
    All of this facts will be made clear in Subsection \ref{sec: stable distributions} below.

    If $f$ is in the critical branching rate regime, i.e., $\alpha \beta = \kappa_f b(1+\beta)$, 
    %then \eqref{eq: equatlity for mf for small rate} doesn't hold anymore. 
...
We have established some central limit theorems in \cite{RenSongZhang2015Central} for a class of superprocesses with general motions under the assumption that the branching mechanisms
satisfy a second moment condition.
    We hope to prove stable CLT's for superprocesses with general motions in a future project.
...
\label{sec: stable distributions}
   ...
   Recall that
    $(P^\alpha_t)_{t\geq 0}$ is defined in \eqref{eq: meansemigroup}, $\mathcal P$ is defined in \eqref{eq: polynomial growth function}, $\mathcal N$ is given by \eqref{eq: def of N} and $\mathcal C_l$ is defined in \eqref{eq: def of Cl}.
\begin{lem}
...
\]
    By the fact that $\langle \bar{g}_t,\varphi\rangle = 0$, we get that for each $s,t\geq 0, x\in \mathbb R^d$,
\begin{align}
    &|P_s\bar{g}_t(x)| 
...
\end{align}
    ...
    %$c, \delta, t_0 > 0$ such that
    $c_2, \delta_1, t_0 > 0$ such that
\[
    |\bar v_t-\bar v|
%    \leq ce^{-\delta t},    \quad t\geq t_0.
...
%    Since the $L\log L$ criterion is satisfied, we have (see \cite{LiuRenSong2009Llog} for
...
\]
%    The $L \log L$ criterion also guarantees that (see again \cite{LiuRenSong2009Llog} 
...
\]
    ...
    %constants $C>0$ and $t_0>0$ such that for all $t\geq t_0$,
...
\[
    ...
    %C(k_t^{\delta}+e^{-\delta t}),\quad t\geq t_0.
...
P.~Mi{\l}o{\'s}, 
%\emph{Spatial {CLT} for the supercritical
