...
Some phase transitions arise  for the central limit theorems in the sense that the forms of the central limit theorem are different in three different regimes corresponding the branching rate being relatively small, large or at a critical balanced value.
\end{abstract}
...
The functional version of the CLT was established in \cite{Janson2004Functional} for supercritical multitype branching processes, and in \cite{RenSongZhang2017Functional} for supercritical superprocesses.
...
\end{align}
and a family of $\mathbb C$-valued functionals 
%Jianjie $(m_t)_{0 \leq s \leq t < \infty}$ on $\mathcal P$ 
$(m_t)_{0 \leq t < \infty}$ on $\mathcal P$ 
by
\begin{align}
  \label{eq:I:R:2}
  m_t[f]
...
\end{align}
Define $ \mathcal C_s := \mathcal P \cap \operatorname{Span} \{ \phi_p: \alpha \tilde \beta < |p| b \}$, $\mathcal C_c   := \mathcal P \cap \operatorname{Span} \{ \phi_p : \alpha \tilde \beta = |p| b \} $
and $ \mathcal C_l   := \mathcal P \cap \overline{\operatorname{Span}} \{ \phi_p: \alpha \tilde \beta > |p| b \}$. For $f\in \mathcal P\setminus \{0\}$, in Lemma \ref{lem:m} and Proposition \ref{prop:PL:S} below, we will show that
\begin{align}
  \label{eq:I:R:3}
  m[f]
  := \begin{cases}
...
%Indeed, considering a general $f \in \mathcal P$ which can be decomposed as $f_s + f_c + f_l$ with $f_s \in \mathcal C_s$, $f_c \in \mathcal C_c\setminus\{0\}$ and $f_l \in \mathcal C_l$, as a simple corollary of Theorem \ref{thm:M},
%one can verify that
%\[
...
A general  $f \in \mathcal P$ can be decomposed as $f_s + f_c + f_l$ with $f_s \in \mathcal C_s$, $f_c \in \mathcal C_c\setminus\{0\}$ and $f_l \in \mathcal C_l$. For $f\in  \mathcal P$
satisfying $\alpha \tilde \beta > \kappa_f b$, $f_s$ and $f_c$ maybe non-trivial.
...
%Jianjie $ U_tf(x) := \operatorname{Log} \mathbb P_{\delta_x}[e^{i\theta \langle f,X_t\rangle}]|_{\theta = 1} $ 
$ U_tf(x) := \operatorname{Log} \mathbb P_{\delta_x}[e^{i\theta X_t(f)}]|_{\theta = 1} $ 
be the value of the characteristic exponent of the infinitely divisible random variable
%jianjie $\langle f, X_t\rangle$ at $1$.
$X_t(f)$
...
\begin{proof}
  ...
  and Lemma \ref{lem: property of controllable operators}.(1) and (4), we get that, for all $\delta > 0$ small enough, the operators
  \[
    f
    \mapsto Z_t'' f
...
%Jianjie add
satisfying Grey's condition and $L \log L$ condition.
%add end 
...
In this subsection,  we want to find some upper bound for the $(1+\gamma)$-th moment of $X_t(g)$,
where $\gamma \in (0,\beta)$ and $g\in \mathcal P$.
...
%Jianjie \[
  $
    C
     := \Big(\frac{2^{2+\beta}}{2+\beta} + \frac{2^{3}}{3} \Big) \mu(Q_0Rh).
    \qedhere
  $
%Jianjie \]
\end{proof}
...
\begin{proof}
  ...
  \begin{align}
     & \mathbb P_\mu\big[|\mathcal I_r^sX_t(g)|^{1+\gamma}\big]
...
%Jianjie Define $ H_t^p :=e^{-(\alpha-|p|b)t}X_t(\phi_p)$ for each $t\geq 0$.
and $ H_t^p :=e^{-(\alpha-|p|b)t}X_t(\phi_p)$ for each $t\geq 0$.
...
\begin{proof}[Proof of Theorem \ref{thm: law of large number}]
  ...
  \item
    ...
    \]
  \item
  if $\gamma \in (0,\gamma_0)$,  then
    thanks to (1)--(3) above and the fact that \[\|e^{-(\alpha - \kappa b)t} X_t(g)\|_{\mathbb{P}_{\mu};1+\gamma}
      \leq \|e^{-(\alpha - \kappa b)t} X_t(g)\|_{\mathbb{P}_{\mu};1+\gamma_0},\] there exist $C_5, \delta_2 >0$ such that
    \[
      \|e^{-(\alpha - \kappa b)t} X_t(g)\|_{\mathbb{P}_{\mu};1+\gamma}
      \leq C_5e^{-\delta_2 t},
      \quad t\geq 0.
...
\begin{align}
  ...
  J^f_1(t,\epsilon)
%Jianjie  & \leq \sum_{\rho \in \{\delta_0,1\}}e^{-\frac{1+\beta+\rho}{1+\beta}(\alpha-\epsilon)t}\mathbb{P}_{\mu}[X_{t-1}(Rh_2)]
...
\begin{proof}[Proof of Theorem \ref{thm:M}.(\ref{thm:M:3})]
  ...
  and even closer so that there exist $\eta,\eta'>0$ satisfying $\alpha \tilde \gamma > \eta>\eta - 3\eta'> \alpha (\tilde \beta - \tilde \gamma)>0$. We also define $\mathcal{D}_{t,k} :=\{|H_t-H_{t+k}|\leq  e^{-\eta t}, H_{t}> 2e^{-\eta' t}\}$.
