...
\end{equation}
    ...
    %Denote by $(P_t^\alpha)_{t\geq 0}$ the transition semigroup of $\xi$.
...
$$H_t^p:= e^{-(\alpha-|p|b)t}\langle\phi_p,X_t\rangle,\quad t\geq 0.$$
  For all $\gamma\in (0, \beta)$ and $\mu\in \mathcal M_c(\mathbb R^d)$, on the condition $\alpha\beta>|p|b(1+\beta)$, $H_t^p$ is a $\mathbb{P}_{\mu}$-martingale bounded in
...
\]
According to \cite[Lemma 5.1]{MM}, $m[f]$ is well-defined.
\begin{thm}
\label{Theorem13}
...
\end{align}
Fixed $f\in \mathcal{C}_l$, $f$ can be expressed by
...
\end{align}
It is easy to get that $I_sf(x)=\mathbb{P}_{\delta_x}[\langle I_t f, X_{t-s}\rangle]=P_{t-s}^{\alpha}I_tf(x)$. Denote
\begin{align}
    \bar{m}[f]:=\int_{0}^{\infty} e^{\alpha s}\langle \eta(iI_sf)^{1+\beta},\varphi\rangle ds.
\end{align}
According to Lemma \ref{lem: control of mn}, $\bar{m}[f]$ is well-defined.
\begin{thm}\label{theorem 1.6}
For each $f\in\mathcal{C}_l$ expressed by \eqref{discompose} and $\mu\in \mathcal{M}_c(\mathbb{R}^d)$, under $\mathbb{P}_{\mu}(\cdot|D^c)$ it holds that,
\begin{align}\label{thm: large rate}
    \frac{\langle f, X_t\rangle-\sum_{p\in\mathcal{N}}a_pe^{(\alpha-|p|b)t}H^p_{\infty}}{\|X_t\|^\frac{1}{1+\beta}}\xrightarrow{d}\bar{\zeta},\quad t\rightarrow\infty,
\end{align}
where $\bar{\zeta}$ is an $(1+\beta)$-stable random variable with characteristic function
\begin{align}
    \mathbb{E}[e^{i\theta\bar{\zeta}}]=\exp[\bar{m}[\theta g]],\quad \theta\in \mathbb{R}.
\end{align}
\end{thm}
...
\subsection{Control the operators for super-OU process.} Let $X$ be the super-OU process introduced in subsection 1.1. Write $\psi_0(z)=\psi(z)+\alpha z$, for each $z\in \mathbb{C}_+$, and define operators on $\mathcal{B}(\mathbb{R}^d,\mathbb{C}_+)$ by  
...
For all $t\in [0,\infty), x\in \mathbb R^d $ and $f \in \mathcal{P}$, recall that $U_tf(x) := \operatorname{Log} \mathbb P_{\delta_x}[e^{i\theta \langle f, X_t\rangle}]|_{\theta = 1}$, the value of the L\'evy exponent of the infinite divisible random variable $\langle f, X_t\rangle$ at $1$.
...
In this section, we will introduce a small value probability result of supercritical continuous-state branching process with general branching mechanism and control $(1+\gamma)$-moments for the super-OU process.
...
\]
    ...
    Fix this $z_0$. For $t$ large enough, according to \eqref{CSBP: int} and the fact that $0<k_t^{-1}e^{-\alpha t} < v_t(k_t^{-1}e^{-\alpha t})< \bar v$, we can verify the following:
\begin{equation}\begin{split}
...
\]
    %where $Q_kg$ is defined by \eqref{Q_k} with $f$ replaced by $g$.
\end{lem}
\begin{proof}
...
\begin{equation}\begin{split}
    ...
    % k is replaced by l to avoide confusing between k and \kappa.
...
\subsection{The large rate case.} To prove Theorem \ref{Theorem11}, we first prove the almost sure and $L^{1+\gamma}(\mathbb{P}_{\mu})$ convergence of a family martingales for $\gamma\in (0, \beta)$. Recall that $L$ is the infinitesimal generator of the OU-process. Let $f\in \mathcal{P}\cap C^2(\mathbb R^d)$, and let $a\in \mathbb R$, we define that
\begin{equation}\begin{split}
\label{defmartingale}
...
% According to Lemma \ref{lem: control of mgtrs},
...
\subsubsection{Proof of Theorem \ref{Theorem11}}.
...
\subsection{The critical rate case.} First we introduce an criterion of $(1+\beta)$-stable distribution.
\begin{lem}
...
\[
    ...
    \quad t\geq 0.
\]
    According to \cite[Lemma 4.2]{MM}, if $f \in \mathcal{P}$ satisfies $\alpha\beta=\kappa_f b(1+\beta)$, the following limit exists:
\begin{equation}\begin{split}
...
$
    ...
    The proofs of \eqref{it: second stable} is similar to that of \eqref{it: first stable}.
\end{proof}
...
\begin{proof}
    ...
    %Fix $t\geq 0$, $n \in \{0,...,\lfloor t \rfloor\}$ and $(\theta_0,...,\theta_n)\in \mathbb R^{n+1}$
...
\subsubsection{Proof of Theorem \ref{Theorem12}}
...
\end{align*}
  ...
  %   $|\mathbb{\tilde{P}}_{\mu}[e^{i\theta I_1(t)}]-\mathbb{\tilde{P}}_{\mu}[e^{i\theta\tilde{I}_1(t)}]|\xrightarrow[t\to \infty]{} 0$
 ...
 \end{equation}
      and
      %$I_i(t)\xrightarrow[t\to \infty]{d} 0$ for $i = 2,3$ under $\tilde {\mathbb P}_\mu$.
...
%\begin{equation}\begin{split}\label{ineq: control of I1t}
...
\subsection{The small rate case.}
     For each $f\in \mathcal{P}$ satisfying $\alpha\beta<\kappa_fb(1+\beta)$, let
\begin{equation}\begin{split}
    m[f]
    :=\sum_{k=0}^\infty m_k[f]
    =\int_0^{\infty} e^{-\alpha s} \langle \eta (-iP_s^\alpha f)^{1+\beta}, \varphi \rangle~ds. 
\end{split}\end{equation}
According to \cite[Lemma 5.1]{MM}, $m[f]$
is well defined, i.e. $|m[f]|<\infty$.
\begin{cor}
	For all $t\geq 0$ and $f\in \mathcal P$ satisfying $\alpha\beta < \kappa_f b(1+\beta)$, 
    $\theta \mapsto \exp(m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable.
\end{cor}
\begin{proof}
Similar to the proof of corollary \ref{cor: alpha stable rv}.
\end{proof}
\subsubsection{Proof of Theorem \ref{Theorem13}}
...
\begin{align*}
    ...
    \quad t\geq 0.
\end{align*}
    Hence, according to \eqref{msmallcase}, we have $\tilde I'_1(t)\xrightarrow[t\to \infty]{d} \zeta$ under $\tilde {\mathbb P}_\mu$.
...
\subsection{Further results for large rate case.}
Recall that
\begin{align}
   \mathcal{N}=\{p\in \mathbb{Z}_+^d:  \alpha\beta>|p|(1+\beta)b\}, 
\end{align}
...
\end{align}
For each $g\in \mathcal{C}_l$ expressed by
    $g(x)=\sum_{p\in \mathcal{N}}a_p\phi_p(x)$,
where $x\in \mathbb{R}^d$. Let
\begin{align}
...
\begin{lem}\label{lem: control of Isg}
 There exists $h\in \mathcal{P}$ such that  for any $t\geq 0$,
\begin{align}
    |I_tg(x)|\leq e^{-(\alpha-Kb)t}h(x),\quad x\in \mathbb{R}^d.
\end{align}
\end{lem}
\begin{proof}
Let $h=\sum_{p\in \mathcal{N}}|a_p\phi_p|$.
\begin{align}
...
therefore,
\begin{align}
    \bar{m}[g]=\sum_{n=1}^{\infty}\bar{m}_n[g].
\end{align}
...
\end{align}
By the fact $\alpha\beta>K(1+\beta)b$, we easily get $|\bar{m}[g]|<\infty$.
\end{proof}
\begin{cor}
\label{cor: alpha stable rv 1}
	For all $n\geq 0$ and $g\in \mathcal C_l$ expressed by \eqref{discompose},
\begin{enumerate}
\item
\label{it: first stable}
...
Similar to the proof of corollary \ref{cor: alpha stable rv}.
\end{proof}
Moreover, by fubini's theorem, we get
...
For each $p\in \mathbb{N}_0$ and $t\geq 0$, recall that $H ^p_t=e^{-(\alpha-|p|b)t}\langle\phi_p,X_t\rangle$ is a martingale corresponding to the $\sigma$-algebra $\{\mathscr{F}_t\}_{t\geq 0}$. According to Lemma 2.8, we get
\begin{align}
    \lim_{u\rightarrow \infty}\langle I_ug,X_{t+u} \rangle=\sum_{k:\alpha\beta>k(1+\beta)b}e^{(\alpha-kb)t}\sum_{|p|=k}H_{\infty}^p, \quad \mathbb{P}_{\mu}\text{-a.s}. 
\end{align}
\end{comment}
...
\end{align}
Let $\theta_{t,p,n}=a_p\frac{e^{-(\alpha-|p|b)n}}{(e^{-\alpha(n-1)}\|X_{t+n-1}\|)^{\frac{1}{1+\beta}}}$ and $A_t(\epsilon)=\{\|X_t\|\geq e^{(\alpha-\epsilon)t}\}$, we easily get 
\begin{align}
    \bar{\gamma}_{t,n}=\sum_{p\in \mathcal{N}}\theta_{t,p,n}(e^{\alpha-|p|b}\langle \phi_p, X_{t+n-1}\rangle-\langle \phi_p, X_{t+n}\rangle).
\end{align}

	Step 1. We will show that for each $\epsilon > 0, n\geq 0$, and $t\geq 0$ we have
\begin{align}
...
\end{align}
 From the definition of $K_1$ and $Z_1$, we have
\begin{align}
\label{eq: need11}
    &\mathbb{P}_{\mu}[e^{i\bar{\gamma}_{t,n}}|\mathscr{F}_{t+n-1}]
    ...
    \\&=e^{\langle (K_1+Z_1)(-\sum_{p\in \mathcal{N}}\theta_{t,p,n}\phi_p), X_{t+n-1} \rangle}.
\end{align}
...
Step 2.  We will show that for each $\epsilon$ small enough, there exists constant $C,\delta>0$ such that,
    \begin{align}
    \label{eq:large rate J1}
      J'_1(t,n,\epsilon)\leq C e^{-\delta(t+n)}, n\geq 1, t\geq 0.
    \end{align}
...
In fact, according to the definition of $Z_1$, $\bar{m}_n$ and \eqref{equ: transform of mn}, we have
\begin{align}
...
\begin{align}
	...
	&\leq e^{\alpha(n-1)}e^{-(\alpha-Kb)(1+\beta)n}e^{-(\alpha-\epsilon)(t+n-1)}\mathbb{P}_{\mu}[\langle g_p, X_{t+n-1}\rangle],
	\quad t\geq 0, n\geq 1.
\end{align}
Where
\begin{align}
    g_n=\frac{Z_1(-I_ng)-\langle Z_1(-I_ng),\varphi\rangle}{e^{-(\alpha-Kb)(1+\beta)n}}.
\end{align}
Similar to proof of \cite[Lemma 2.8]{MM}, there exists $h \in \mathcal P$, such that
\[
     |T_t g_n|\leq e^{-bt}h,
     \quad t\geq 0, n\geq 1.
\]

 Chose $\gamma\in(0,\beta)$ small enough such that $\alpha \gamma < b < (1+\gamma)b$. According to Lemma \ref{lem: control moment} (3) with $\kappa=1$, there exists $C>0$ such that 
\begin{align}
    \mathbb{P}_{\mu}[\langle g_n, X_{t+n-1}\rangle]\leq Ce^{\frac{\alpha \gamma}{1+\gamma}(t+n-1)},\quad t\geq0, n\geq 1.
\end{align}
Therefore, 
\begin{align}
    &J'_2(t,n,\epsilon)\leq e^{\alpha(n-1)}e^{-(\alpha-Kb)(1+\beta)n}e^{-(\alpha-\epsilon)(t+n-1)}\mathbb{P}_{\mu}[\langle g_n, X_{t+n-1}\rangle]\\
...
\end{align}
	Step 4. We will show that for each $\epsilon< \alpha$ there exist $C,\delta>0$ such that
\begin{align}\label{ineq: control of J31}
    J'_3(t,n,\epsilon)\leq Ce^{-\delta (t+n)}\quad t\geq0, n\geq 1.
\end{align}
	Similar to the proof of step 4 in Lemma \ref{lem: mainlemma}
...
\begin{lem}\label{lem:lemma03}
    For each $\theta\in \mathbb{R}$ and $\mu \in \mathcal{M}_c(\mathbb{R}^d)$, there exist $C,\delta>0$ such that for any $t\geq 0$, $m\in\mathbb{N}$, we have
...
\end{lem}
\begin{proof}
Step 1. Similar to the proof of Corollary \ref{cor: used in next corollary}, we can show that  For each  $\theta\in \mathbb{R}$ and $\mu\in \mathcal{M}_c(\mathbb{R}^d)$, there exist $C,\delta>0$ such that for any $t\geq 0$, $n\geq 1$, we have
\begin{align}
...
Step 2.   Fix $t\geq 0$, $m\in \mathbb{N}$ and $\theta\in \mathbb{R}$, 
    ...
    \end{align*}
    According to step 1 and corollary \ref{cor: alpha stable rv 1} there exist $C,\delta>0$ such that for any $n=1,..m$ and $t\geq0$, we have
    \begin{align*}
        &|\tilde{a}_{n-1}- \tilde{a}_n|
...
\begin{lem}\label{lem: lemma04}
     For each $\theta\in \mathbb{R}$ and $\mu\in \mathcal{M}_c(\mathbb{R}^d)$, there exist $C,\delta>0$
...
\end{align}
Step 1. Similar to the proof of step 1 in section \ref{sec: proof of thm 1.3}, we can show that there exist $C,\delta >0$ such that for all $n \geq 1$ and $t\geq 0$,
\begin{align}
\label{thm12111}
...
\end{align}
    This implies the desired result in this step, since $|Y_{t,k}| \leq 2$ a.s..
\end{comment}
    Step 2. We will show that for each $\mu \in \mathcal{M}_c(\mathbb{R}^d)$, there exist constant $C,\delta > 0$, such that for all $n\geq 1$ and $t\geq 0$,
\begin{align}
\label{thm12211}
...
\end{align*}
\end{comment}
Since $\eta_1 < \eta_0$,  Similar to the proof of step 2 in section \ref{sec: proof of thm 1.3}, we can show that, there is a constant $C\geq 0$ such that,
\begin{align}
...
\begin{comment}
 ...
 Step 3. We will show that there exist $C, \delta> 0$ such that
    \begin{align}
    \Big|\tilde{\mathbb{P}}_{\mu}[\prod_{n=1}^me^{i\theta \tilde{\gamma}_{t,n}}]-\tilde{\mathbb{P}}_{\mu}[\prod_{n=1}^me^{i\theta \bar{\gamma}_{t,n}}]\Big|\leq Cm e^{-\delta t},\quad m\in \mathbb{N}, t\geq 0.
    \end{align}
\end{comment}  

    Combine \eqref{ineq: used next 3}, Step 1 and Step 2, we get the result immediately. Notice that $\delta$ is indenpendent of $m$ and $t$.
\end{proof}
\begin{comment}

Let 
\begin{align}
    \gamma_t:=\frac{H_t^p-H_{\infty}^p}{e^{-(\alpha-|p|b)t}\|X_t\|^{\frac{1}{1+\beta}}},\quad t\geq 0, p\in \mathbb{N}_0^d.
\end{align}
\end{comment}
...
\end{lem}
\begin{proof}
In fact, recall that $A_t(\epsilon):=\{\|X_t\|>\|\mu\|e^{(\alpha-\epsilon) t}\}$. According to proposition \ref{lem: control of XT}, there exists
...
\begin{align}\label{ineq: control of gamma_t on d}
  ...
  & \theta\in\mathbb{R}, m\in \mathbb{N}, \epsilon>0, t\geq 0.
\end{align}
...
\subsubsection{Proof of Theorem \ref{theorem 1.6}}

Let $\delta_0$ be the control parameter in Lemma \ref{lem: lemma04} and fixed $m=\lfloor e^{\frac{\delta_0}{2}t}\rfloor$, notice that
\begin{align}
...
\end{align}
we complete the proof by letting $t\rightarrow\infty$.
\appendix
\section{}
