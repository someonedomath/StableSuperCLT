...
To present our result we need to define a family of operators $(T_t)_{t\geq 0}$ on $\mathcal P$ by
    ...
    %YX f \in \mathcal C_s \oplus \mathcal C_l \setminus \{0\}\\
...
%YX then the right hand side of \eqref{eq: equatlity for mf for small rate} may not converge.
%YX Instead, see \eqref{CLT:C:eq:m} below, there is another quantity $m[f]$ satisfying that
then
\[
  m[f]
...
\]
%YX where $\zeta$ is another $(1+\beta)$-stable random variable with characteristic function $\theta\mapsto \exp( m[\theta f])$.
...
%YX For a general $f$, we have a unique decomposition: $f=f_l+f_c+f_s$,
...
%YX In Subsection \ref{sec:critical}, we will give the proof of Theorem \ref{thm:M}.
...
%YX Recall that the operators $(T_t)_{t\geq 0}$ are given by \eqref{eq:I:R:1},
...
\begin{proof}
  ...
  %YX there exists an $h \in \mathcal P$ such that
...
where
    ...
    %YX there exists an $h_0 \in \mathcal P$ such that for each $t\geq 0$,
 ...
 %YX  an $h \in \mathcal P$ such that
 ...
 %YX Also note that in step 2, we already show that there exists a $\delta > 0$ and an $h \in \mathcal P^+$
...
%YX  we know that there exists a $\delta \geq 0$   and an $h \in \mathcal P$ such that $|T_sf| \leq e^{- \delta t} h$ for each $s\geq 0$.
 ...
 %YX Suppose that $f \in \mathcal P \setminus (\mathcal C_s \oplus \mathcal C_l)$,
...
%YX  According to \eqref{eq:P:S:1} we have that in this case $m[\theta f]$ also admits an integral representation
...
%YX an $h \in \mathcal P^+$ such that for each
...
and $t\geq 0$, we have
\(
%YX \big | P_t Z_1 f - \langle Z_1 f, \varphi \rangle \big|
%YX \leq e^{-bt} h.
...
$f\in \mathcal P_g$,  there exists  $h \in \mathcal P^+$ such that for each $|P_tq_f| \leq e^{-bt} h.$
  ...
  \emph{Step 1.}
  %YX We argue that we only have to proof the result for all
  We claim  that we only need to prove the result for all
  $f \in \widetilde{\mathcal P}_g:= \{T_{n+1} g : n \in \mathbb Z_+\}$.
%YX  In fact, writing $q_f:=Z_1f - \langle Z_1f, \varphi \rangle \in \mathcal P^*$ for each $f\in \mathcal P$, then both $\operatorname{Re} q_g$ and $\operatorname{Im} q_g$ are functions in $\mathcal P$ of order $\geq 1$.
 In fact, both $\operatorname{Re} q_g$ and $\operatorname{Im} q_g$ are functions in $\mathcal P$ of order $\geq 1$.
%YX Therefore, the result is valid for $f = T_0 g = g$.
...
\]
%YX where $g = g_0 +g_1$ with $g_0 \in \mathcal C_s \oplus \mathcal C_c$ and $g_1 \in \mathcal C_l$.
...
%YX an $h_3 \in \mathcal P^+$ such that
...
\end{align}
%YX Write $g = g_0 + g_1$ with $g_0 \in \mathcal C_s \oplus \mathcal C_c$ and $g_1 \in \mathcal C_l$.
...
\]
%YX Therefore, there exists an $h'_3\in \mathcal P^+$
...
%YX an $h'_4\in \mathcal P^+$ such that
...
\begin{align}
  & |\partial_j  P_{1-u}^\alpha (- i P_u^\alpha f)^{1+\beta}|
%YX  \leq (1+\beta) e^{\alpha (2+\beta)} P_{1-u} [  (P_u |f|)^\beta P_u |\partial_j f| ]
%YX \\&    \leq (1+\beta) e^{\alpha (2+\beta)} P_{1-u} [  (P_u h'_4)^\beta P_u h_3 ]
%YX   \leq (1+\beta) e^{\alpha (2+\beta)} Q_0 [  (Q_0 h_4')^\beta Q_0 h_3 ],
...
\emph{Final step.}
%YX From Step 5, there are $c_5,n_5> 0$ such that for each $x\in \mathbb R^d$, $h_5(x) \leq c_5(1+|x|)^{n_5}$.
...
Write %YX $q_f = Z_1f - \langle Z_1f, \varphi\rangle$ for each $f\in \mathcal P$; and
$D_{x,y} = \{ax+by: a, b \in [0,1]\}$ for each $x, y \in \mathbb R^d$.
Using \eqref{eq:P:R:3:-1} and Step 5,
%YX there exists an $h_6 \in \mathcal P^+$
...
%YX & \leq c_0 e^{-bt}(1+|x|)^{n_0}\Big(|x|\int_{\mathbb R^d} (1+|y|)^n\varphi(y) ~dy + \int_{\mathbb R^d} (1+ |y|)^n |y| \varphi(y) ~dy \Big) \\
...
%YX Recall that we also write $\tilde u = \frac{u}{1+u}$ for each $u\neq -1$.
...
%YX Moreover if $\alpha\tilde \beta>|p|b$, the martingale is bounded in $L^{1+\gamma}(\mathbb P_\mu)$ for each $\gamma\in (0, \beta)$
    Moreover if $\alpha\tilde \beta>|p|b$, the martingale is bounded in $L^{1+\gamma}(\mathbb P_\mu)$ for each $\gamma\in (0, \beta)$.
%YX  As a consequence, $ H_{\infty}^p := \lim_{t\rightarrow \infty}H_t^p $  convergence almost surely  and also in $L^{1+\gamma}(\mathbb{P}_{\mu})$ for each $\gamma \in (0,\beta)$.
...
\begin{proof}
  ...
  %YX there exists a constant $c>0$ such that
...
%YX = c
  ...
  , \quad t\geq 0.
\]
%YX For each $\gamma\in (0, \gamma_0)$ there exists a constant $c>0$, such that
...
\begin{proof}
%YX   Thanks to Lemma \ref{lem:M:L:ML} we only have to proof this when $0\leq s < t<\infty$.
 Thanks to Lemma \ref{lem:M:L:ML}, we only  need to prove the inequality  when $0\leq s < t<\infty$.
%YX  Fix the $p\in \mathbb{Z}_+^d$, $\mu\in \mathcal M_c(\mathbb R^d)$ and the $\gamma \in (0,\beta)$ with $\alpha \tilde \gamma > |p|b$.
 Suppose $p\in \mathbb{Z}_+^d$, $\mu\in \mathcal M_c(\mathbb R^d)$ and  $\gamma \in (0,\beta)$ with $\alpha \tilde \gamma > |p|b$ are fixed.
 %YX By Lemma \ref{lem: control of mgtrs} with $g=\phi_p$ and $k=|p|$,
...
%YX \subsection{Local central limit theorems}
\subsection{Central limit theorems for one unite time interval}
\label{sec:critical}
% ***** Local CLT for one interval for small and critical
%YX In this section, we will establish the following local central limit theorem for our super OU processes.
...
%YX In fact, we can proof a stronger result:
...
\end{align}
%YX From Proposition \ref{prop:PL:S} and Lemma \ref{lem: charactreisticfunction}, we easily get $\theta\mapsto \langle Z_1(\theta f),\varphi\rangle$ is the characteristic function of some $(1+\beta)$-stable random variable. Therefore  $\operatorname{Re} \langle Z_1f, \varphi\rangle \leq 0$.
...
%YX Acording to the step 1 of the proof of Lemma \ref{lem:m},
Acording to Step 1 of the proof of Lemma \ref{lem:m},
%YX there exist $h_{20} \in \mathcal P^+$ such that for each $f \in \mathcal P_g$ it holds that $|f| \leq h_{20}$.
...
\begin{align}
 ...
 %YX   \leq R(|\theta_{t} f|)\mathbf{1}_{A_{t-1}(\epsilon)}
 %YX \leq R \Big(\frac{h_{20}}{e^{(\alpha-\epsilon)(t-1)(1-\tilde \beta)}}\Big) \\
 %YX  & \leq \sum_{\rho \in \{\delta_0,1\}}e^{-\frac{1+\beta+\rho}{1+\beta}(\alpha-\epsilon)(t-1)}Rh_{20}.
...
\begin{align}
  ...
  J^f_1(t,\epsilon)
%YX & \leq \sum_{\rho \in \{\delta_0,1\}}e^{-\frac{1+\beta+\rho}{1+\beta}(\alpha-\epsilon)t}\mathbb{P}_{\mu}[\langle Rh_0,X_{t-1}\rangle]\\
%YX  & \leq \sum_{\rho \in \{\delta_0,1\}} \langle Q_0 R h_{20}, \mu \rangle e^{-(\alpha\frac{\rho}{1+\beta}-\epsilon\frac{1+\beta+\rho}{1+\beta})t},
...
\)
%YX It follows from Lemma \ref{lem:P:R} that there exists an $h_{30}\in \mathcal{P}$ such that for each $ f\in \mathcal P_g$, we have
...
%YX In the rest of this step, fixing a $\gamma\in(0,\beta)$ small enough such that $\alpha \gamma < b < (1+\gamma)b$.
...
%YX there exists $C_{30}>0$ such that for all $t\geq 0$ and $f\in \mathcal P_g$,
...
%YX  & \leq 2\sup_{q\in \mathcal P: Q_1 q\leq h_{30}} \|\langle q, X_t\rangle\|_{\mathbb P_\mu; 1+\gamma} \leq C_{30} e^{\frac{\alpha t}{1+\gamma}}.
...
%YX \leq C_{30} e^{-(\alpha\tilde \gamma -\epsilon)t}.
...
%YX there exists  $C_{40}, \delta_{40}>0$ such that for all $t\geq 0$,
there exists  $C_{4}, \delta_{4}>0$ such that for all $t\geq 0$,
\begin{equation}
  \mathbb P_\mu(A_t(\epsilon)^c,D^c)
  \leq \mathbb P_\mu(0 < e^{-\alpha t}\|X_t\|
%YX  \leq e^{ - \epsilon t}) \leq C_{40} (e^{-\epsilon \delta_{40} t}+e^{-\delta_{40} t}).
     \leq e^{ - \epsilon t}) \leq C_{4} (e^{-\epsilon \delta_{4} t}+e^{-\delta_{4} t}).
\end{equation}
%YX Combining these results, we get the desired result in step 4.
 Combining these results, we get the desired result in this step.

%YX \emph{Final step}, combining the results in Steps 1--4, we immediately get the desired result.
 \emph{Final step}. Combining the results in Steps 1--4, we immediately get the desired result.
\end{proof}
% ***** Local CLT

% ***** local CLT one interval for large
% ***** LCLT for conditional one intervald
...
\begin{proof}
  ...
  %YX for each $l\leq n$ in $\mathbb Z_+$, each $k \in \{l,\dots , n\}$, and each $(f_j)_{j=l}^n\subset \mathcal P_g$, we have
  ...
  \begin{align}
    ...
    \leq C_0 e^{-\delta_0 k}.
  \end{align}
 %YX Therefore, there exist $C_1,\delta_1 >0$ such that
...
%YX      \leq C_1 e^{- \delta_1 l}.
         \leq C e^{- \delta l}.
      \qedhere
  \end{align}
\end{proof}

% ***** local CLT multiple interval for large branching rate
...
%YX  Therefore from Proposition \ref{cor:MI} we get that there exist $C_1,\delta_1 > 0$ such that
  ...
  %YX of semigroup $(P_t)$, we can verify that
...
%YX  In fact,  by \cite[Lemma 3.4.3]{Durrett2010Probability} we can write for each $t\geq t_0$ that
...
%YX $|\mathbb{\widetilde{P}}_{\mu}[e^{i (I^f_1(t)- I^f_0(t))}]| \leq C_2't^{-\delta_1}$ which, combines with the fact that $I^f_1(t) - I^f_0(t)$ is linear in $f$, completes this step.
...
%YX We will show this claim in the following substep 2.1 and 2.2.
...
%YX	\emph{Final step}, combining Steps 1--4, we complete the proof of \ref{thm:M}.\eqref{thm:M:1}.
...
%YX    For each $t\geq t_0$, in this proof we write $\theta_t = \|t X_t\|^{\tilde \beta - 1}$; and we define $I_i^f(t)$ for $i = 1,2,3$ using \eqref{eq:PM:CLTS:1}, and set
...
% ****** Step 1.
  ...
  \begin{align}
    ...
    \quad t\geq t_0.
  \end{align}
%YX  Note that since $f \in \mathcal C_c\setminus \{0\}$, so $T_k \tilde f = \tilde f$ for each $k \in \mathbb Z_+$.
...
% ****** Step 2.
  \emph{Step 2.} We show that $ I^f_1(t) - I^f_0 (t) \xrightarrow[t\to \infty]{d} 0$.
%YX  In fact, similar to the Step 2 of the proof of Theorem \ref{thm:M}.(\ref{thm:M:1}), we also have \eqref{eq:PM:S:1} is valid with
...
\)
%YX  We also claim that there exists $C_2, \delta_2>0$ such that \(\widetilde {\mathbb P}_\mu [|Y_{t,k}|] \leq C_2 e^{-\delta_2 (t-k-1)}\) for each $k\in \mathbb N$ and $t\geq k+1$; and similarly, the desired results in this step follows with this claim.
...
% ******* subStep 2.1
  \emph{Substep 2.1.}
  %YX  Similar to the substep 2.1 in the proof of Theorem \ref{thm:M}.(\ref{thm:M:1}), we have that there exist $C_{2.1},\delta_{2.1} >0$ such that for all $k \in \mathbb N$ and $t\geq k+1$,
...
In fact, noticing that for $f\in \mathcal C_c$ and $k\in \mathbb Z_+$, we have $T_kf = e^{\alpha (\tilde \beta - 1 )k}P_k^\alpha $; and therefore for each $k\in \mathbb Z_+$ and $t \geq k + 1$,
  ...
  \]
%YX  The rest is similar to the substep 2.2 in the proof of Theorem \ref{thm:M}.(\ref{thm:M:2}).
...
\\
  ...
  \end{align}
%YX  From this and \eqref{Theorem123}, combined with the fact that $I^f_2(t)$ is linear in $f$, we get the desired result in this step.
From this and \eqref{Theorem123}, we get the desired result in this step.

% ****** Step 4
  \emph{Step 4.}
 %YX Similar to the Step 4 of the Proof of Theorem \ref{thm:M}.(\ref{thm:M:1}), we can verify that $I_3(t) \xrightarrow[t\to \infty]{\widetilde {\mathbb P}_\mu \text{-} a.s.} 0$.
...
%YX of  semigroup $(P_t)_{t\geq 0}$ we can verify that
of the semigroup $(P_t)_{t\geq 0}$ we have
\begin{align}
  \label{eq:PM:CLTS:2}
  & \sum_{k=0}^\infty \langle Z_1 T_{k} (-\tilde f), \varphi \rangle
  ...
  = \int_0^\infty \langle  (iT_{u} f)^{1+\beta}, \varphi\rangle~du = m[-f].
\end{align}
%YX Now, combining with the fact that $I_0^f(t)$ is linear in $f$, the result in this step follows.
The result in this step follows.

\emph{Step 2.} We show that $I^f_1(t) - I^f_0(t) \xrightarrow[t\to \infty]{d} 0$.
%YX In fact, by \cite[Lemma 3.4.3]{Durrett2010Probability} we can write for each $t\geq 0$ that
...
%YX which, combined with the fact that $I_1^f(t) - I_0^f(t)$ is linear in $f$, completes this step.
which completes this step.

%YX We will show this claim in the following substeps 2.1 and 2.2.
 We will show this claim in the following Substeps 2.1 and 2.2.
%YX First we chose $\gamma\in(0,\tilde \beta)$ close enough to $\beta$
...
\]
%YX Similar to the substep 2.2 of
...
%YX which combined with the fact that $I_2^f(t)$ is linear in $f$, completes this step.
which completes this step.
