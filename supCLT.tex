...
% **** (fluctuation result of large numbers)
% DELETED
\begin{comment}
...
and in Lemma \ref{prop:PL:S} that, when $f\in \mathcal C_l$ is non-trivial, $\theta \mapsto \exp( \bar m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable.
\begin{thm}
  \label{thm: large clt}
...
We will prove in Lemma \ref{prop:PL:S} that $\theta \mapsto \exp( \widetilde m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable.
\begin{thm}
...
We will prove  in Lemma \ref{prop:PL:S} that $\theta \mapsto \exp( m[\theta f])$ is the characteristic function of a $(1+\beta)$-stable random variable.
\begin{thm}
  \label{thm: small clt}
...
\begin{lem}{\cite[Lemma 2.8]{MarksMilos2018CLT}}\label{lem:P:R}
...
% DELETED
\begin{comment}
\begin{lem}{\cite[Lemma 2.8]{MarksMilos2018CLT}}
  For each $f\in \mathcal{P}$, let
  \begin{align}
    g_k
    := \frac{Z_1 P^{\alpha}_k f-\langle  Z_1P^{\alpha}_k f,\varphi\rangle}{e^{(\alpha-\kappa_f b)(1+\beta)t}},
    \quad k \geq 0.
  \end{align}
  Then there exists an $h\in \mathcal{P}$ such that $|P_sg_k|\leq e^{-bs}h$ for all $s, k \geq 0.$
\end{lem}
...
\begin{lem}
  ...
  \begin{align}
    \bar{g}_t
    :=\frac{Z_1(T_tg)-\langle Z_1(T_tg),\varphi\rangle}{e^{-(\alpha-Kb)(1+\beta)t}},\quad t\geq 0.
  \end{align}
  Then there exists an $h\in \mathcal{P}$ such that $|P_s\bar{g}_t| \leq e^{-bs}h$ for all $s, t\geq 0.$
\end{lem}
\begin{proof}
  First we claim that there exists $h_1\in \mathcal P$ such that
\begin{align}
  \label{ineq: control of partial derivative of Itg}
  \Big|\frac{\partial}{\partial x_i}T_tg(x)\Big| \leq e^{-(\alpha-Kb)t}h_1(x),
  \quad  t\geq 0, x\in \mathbb R^d.
\end{align}
In fact, if we put $h_1(x)=\sum_{p\in \mathbb Z^d_+: \alpha \tilde \beta > |p|b }|\langle g, \phi_p\rangle_\varphi\frac{\partial \phi_p}{\partial x_i}(x)| \in \mathcal P$, then there exists a $\delta > 0$ such that
\begin{align}
  \Big|\frac{\partial}{\partial x_i}T_tg(x)\Big| =
  \Big|\sum_{p\in \mathbb Z^d_+:\alpha \tilde \beta > |p|b}\langle g, \phi_p\rangle_\varphi e^{-(\alpha\tilde \beta-|p|b)t} \frac{\partial}{\partial x_i}\phi_p(x)\Big|
  \leq e^{-\delta t}h_1(x)
  , \quad t\geq 0, x\in \mathbb R^d.
\end{align}
It is well known that
...
\[
  \frac{\partial P_t^\alpha f(x)}{\partial x_i} = e^{-bt} P_t^\alpha\Big(\frac{\partial f}{\partial x_i}\Big)(x), \quad t\geq 0, x\in \mathbb R^d.
\]
Note that if $g \in \mathcal C_l$, then $g$ is a linear combination of polynomials.
Therefore, $g^{1+\beta}\in \mathcal P$ and $|\nabla (g^{1+\beta})| \in \mathcal P$.
Therefore,
\begin{align}
  & \frac{\partial}{\partial x_i} P_{1-s}^\alpha  (P_s^\alpha I_t g)^{1+\beta}
    = \frac{\partial}{\partial x_i} P_{1-s}^\alpha (I_{t-s} g)^{1+\beta}
...
Using this and \cite[Theorem A.5.2]{Durrett2010Probability}, we can get
\[
  \frac{\partial}{\partial x_i} Z_1(T_tg)
  = \eta i^{1+\beta}  \frac{\partial}{\partial x_i} \int_0^1 P_{1-s}^\alpha (I_{t-s}g)^{1+\beta} ds
  = \eta i^{1+\beta}  \int_0^1  \frac{\partial}{\partial x_i} P_{1-s}^\alpha (I_{t-s}g)^{1+\beta} ds.
\]
Therefore, using \eqref{eq: key estimation for partial} and the above, there exists  $C> 0$ such that for all $t\geq 0$,
...
\begin{lem}
  ...
  % Moreover, if $\alpha\beta>|p|b(1+\beta)$, then for all $\gamma\in (0, \beta)$, we have $\sup_{t\geq 0}\|H_t^p\|_{\mathbb P_\mu;1+\gamma}< \infty$, 
  Moreover if $\alpha\tilde \beta>|p|b$, then the martingale is bounded in $L^{1+\gamma}(\mathbb P_\mu)$ for each $\gamma\in (0, \beta)$. 
  % and $ H_{\infty}^p := \lim_{t\rightarrow \infty}H_t^p $ exists $\mathbb{P}_{\mu}$-a.s and in $L^{1+\gamma}(\mathbb{P}_{\mu}).$
...
\begin{proof}
  ...
  % Using  Lemma \ref{lem:P:M:uc} and the fact $\kappa_{\phi_p}=|p|$, we get that, for each $\gamma\in [\gamma_0, \beta)$, there exists a $C_{\gamma, \mu, p}>0$ (depending on $\gamma$, $\mu$ and $p$) such that
...
\begin{equation}
    \gamma_{t,k}
    := \frac{\mathcal I_{t-k-1}^{t-k}\langle f ,X_t\rangle}{(e^{\alpha (k+1)}\|X_{t-k-1}\|)^{\frac{1}{1+\beta}}}\xrightarrow{d}\zeta_k, \quad t\rightarrow \infty, \label{eq:PR:LC:1}
\end{equation}
...
\end{align}
From Proposition \ref{prop:PL:S}.(1), we have $\operatorname {Re} m_t[f] < 0$. 
...
\end{align}
where $Q$ is defined by \eqref{eq:Q}.
...
\[
  ...
  \quad k \geq 0.
\]
It follows from Lemma \ref{lem:P:R} that there exists $h\in \mathcal{P}$ such that
\[
  Q_1 (\operatorname{Re} g_k) \leq h
  \text{ and } Q_1 (\operatorname{Im} g_k)\leq h,
  \quad k \geq 0,
\]
where $Q_1$ is defined by \eqref{eq:Q} with $\kappa=1$.

Chose a $\gamma\in(0,\beta)$ small enough such that $\alpha \gamma < b < (1+\gamma)b$.
According to Lemma \ref{lem:P:M:uc}.(3) (with $\kappa=1$), there exists $C_3>0$ such that for all $t\geq 0$ and $k\geq 0$,
\begin{align}
...
\begin{proof}[Proof of Theorem \ref{thm: critical clt}]
  ...
  Taking $\theta_k=t ^{-\frac{1}{1+\beta}} \theta $ and $n={\lfloor t-\ln t \rfloor}$ in Proposition \ref{cor:MI}, we get that there exist $C_1,\delta_1>0$ such that,
  \begin{align}
...
Recall that $H^p_t$ are defined before Lemma \ref{lem:M:L:ML}. We write $a_p:= \langle \phi_p, g\rangle_\varphi$ for each $p \in \mathbb Z_+^d$.

\begin{lem}
  \label{large-central}
  For all $n\ge 1$ and $\mu\in\mathcal{M}_c(\mathbb{R}^d)$, under $\mathbb{P}(\cdot|D^c)$, if $f \in \mathcal C_l$, then
\begin{align}
  \gamma^f_{t,n}
  := \frac{X_{t+n+1}(P^\alpha_{-(n+1)}f) - X_{t+n}(P_{-n}^\alpha f)}{\|e^{- \alpha n}X_{t+n}\|^{1 - \tilde \beta}}
  \xrightarrow[t\to \infty]{d}\zeta^f_{n,n+1}.
\end{align}
\end{lem}
\begin{proof}
  We only need to show that
  \begin{align}
    \mathbb{P}_{\mu}[\exp(i\gamma^f_{t,n}); D^c]
    \xrightarrow[t\rightarrow \infty]{}\mathbb{P}_{\mu}(D^c)\exp(m_{n,n+1}[g]).
  \end{align}
 ...
 \theta_{t,p,n}\phi_p), X_{t+n-1}\rangle-\bar{m}_n[g]\big|;A_{t+n-1}(\epsilon)\Big]+ J'_3(t,n,\epsilon) \\
  ...
  Using Lemma \ref{control of gn} and Lemma \ref{lem:P:M:uc}.(3) with $\kappa=1$, there exists $C_3>0$ such that
  \begin{align}
  ...
  Step 4. According to the Step 4 of the proof of  Lemma \ref{lem:PR:LC}, we have for any $\epsilon\in (0,  \alpha)$ there exist $C_4,\delta_3>0$ such that $ J'_3(t,n,\epsilon)\leq C_4e^{-\delta_3 (t+n)}$ for all $ t\geq0, n\geq 1$.
...
% ***** LCLT for conditional one interval
% DELETED
\begin{comment}
\begin{cor}
\label{cor: used in next corollary}
  Suppose $f\in \mathcal{P}$ is non-trivial and satisfies $\alpha\beta\leq \kappa_fb(1+\beta)$.
  Then for all $\Theta >0$ and $\mu\in \mathcal M_c(\mathbb R^d)$, there exist $C, \delta>0$ such that for $k \geq 0, t\geq k+1$ and $|\theta|\leq \Theta$,
\begin{equation}
  \mathbb{P}_{\mu}\Big[\big|\mathbb{P}_{\mu}[e^{i\theta\gamma^f_{t,k}}-e^{m_{k,k+1}[\theta f]}; D^c | \mathscr F_{t-k-1}]\big|\Big]
  \leq Ce^{-\delta(t-k)}.
\end{equation}
\end{cor}
\begin{proof}
	Define $J_1^f(t,k,\epsilon), J_2^f(t,k,\epsilon)$ and $J_3(t,k,\epsilon)$ as in \eqref{eq: Def of Ji}.
  According to \eqref{eq: inequality that will used later},  we have for all $\theta\in \mathbb R$, $k\geq 0$, $t\geq k+1$ and $\epsilon> 0$,
  \begin{align}
    & \mathbb{P}_{\mu}\Big[\big| \mathbb{P}_{\mu}[e^{i\theta \gamma^f_{t,k}}-e^{m_k[\theta f]}; D^c | \mathscr F_{t-k-1}]\big|\Big]
    \leq J^{\theta f}_1(t,k,\epsilon) + J^{\theta f}_2(t,k,\epsilon)+J_3(t,k,\epsilon).
  \end{align}
...
\begin{proof}
  ...
  \end{align}
  According to Step 1 and Proposition \ref{prop:PL:S}, there exists $C_2>0$ such that for any $n=1,\cdots, m$ and $t\geq 0$, we have
  \begin{align}
    | \widetilde{a}_{n-1}- \widetilde{a}_n|
...
\)
  In fact, noticing that for $f\in \mathcal C_s$ and $k\in \mathbb Z_+$, we have $T_kf = e^{\alpha (\tilde \beta - 1 )k}P_k^\alpha $; and therefore for each $k\in \mathbb Z_+$ and $t \geq k + 1$, 
  \[
