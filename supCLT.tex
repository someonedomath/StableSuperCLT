...
\end{equation}
%deleted
%According to \cite[section 2]{Li2011Measure-valued}, the above is equivalent to
...
%end moved and modified from below
    ...
    %It is known that $\xi$ has an invariant density
...
\end{equation}
%moved to above
    %In tihs subsection, we recall some results on the spectrum in $L^2(\varphi)$ of the operator $L$, more details can be found in \cite{GD}.
%end moved to above
...
\end{equation}
    ...
    %According to \cite[Fact 1.2]{MM}, for any $f \in \mathcal{P}$, there exists $R \in \mathcal{P}$ such that for any $t\geq 0$,
...
\subsection{Characteristic functions}
    %Let $X$ be a super OU-process with initial configuration $\mu$.
%new added
...
%end new added
    %For any $f \in \mathcal{P}$, replace $f$ by $-i\theta f$ in equations of \eqref{super} and \eqref{eq2}, we obtain that
...
%delete
%\begin{equation}\label{characteristic}
    %\mathbb{P}_{\mu}[e^{i\theta X_t(f)}]
    %=e^{\langle w_f(\cdot,t,\theta),\mu\rangle},
    %\quad t\in [0,\infty), \mu\in \mathcal M(\mathbb R^d), f\in \mathcal P, \theta \in \mathbb R.
%\end{equation}
    %where $w_f(x,t,\theta)$ satisfies the equation
...
%For notational convenience, we always fix $g\in \mathcal{P}$ in the following sections, correspondingly, there exists $R\in\mathcal{P}$ such that
...
Consider an incremental decomposition of $\langle g,X_t \rangle$.
    %Denote by $\{\mathcal{F}_t:t\geq 0\}$ the filtration of $X$, that is $\mathcal{F}_t=\sigma(X_s:s\in [0,t])$.
    For each $t\geq 0$, denote by $\{\mathcal{F}_t:t\geq 0\}$ the natrual filtration of $X$, that is $\mathcal{F}_t=\sigma(X_s:s\in [0,t])$.
    %For $k \in \mathbb{N}$, let $g_k=T^{\alpha}_k g$.
    For each $f\in\mathcal{P}$ and $k \in \mathbb{N}$, let $f_k=T^{\alpha}_k f$.
    %For each $t-k\geq 1$, according to \eqref{eq:meanformula}, write
    %For each $k \in \mathbb N$ and $t\geq k+1$, 
...
\end{align*}
    %If $k=t$, let $M_t^t[g]:=\langle g_t,\mu\rangle$, where $\mu $ is the initial configuration.
...
%For any $k \in \mathbb{N}$, $x\in\mathbb{R}^d$ and $\theta\in \mathbb{R}$, we have
...
%end comment
%delete
    %Recall some result about the Kuznestov measures of $\{X_t, t\geq 0\}$. Let $\mathbb{D}$ be the collection of $\mathcal{M}(\mathbb{R}^d)$-valued right continuous functions $t\rightarrow \omega_t$ on $(0,\infty)$ having zero as a trap. Denote by $\{\omega_t, t\geq0\}$ the coordinate process of $\mathbb{D}$ and $(\mathcal{C},\mathcal{C}_t)$ the natural $\sigma$-algebras on $\mathbb{D}$ generated by the coordinate process.

    %It's known by \cite[section 8.4]{Li2011Measure-valued} that  one can associate with $\{\mathbb{P}_{\delta_x}:x\in\mathbb{R}^d\}$ a family of $\sigma$-finite measures $\{\mathbb{N}_x:x \in \mathbb{R}^d\}$ defined on $(\mathbb{D},\mathcal{C})$ such that $\mathbb{N}_x\{0\}=0$,
...
%\end{align*}
    %We refer $\{\mathbb{N}_x:x \in \mathbb{R}^d\}$ as {\em Kuznestov measures} of $X$. For more general definition and further properties of such measures, we refer our readers to \cite{Li2011Measure,DK}.
...
%\end{align*}
    %See \cite[Theorem 8.24]{Li2011Measure-valued} for a proof.
...
%There exists $R_1 \in \mathcal{P}$ and $p_1 >1+\beta$ such that for any $k \in \mathbb{N}$ and $0\leq s \leq 1$, we have
...
\end{lem}
\begin{proof}
%As $\exp({\rm Re~}w_{g_k})=|\exp(w_{g_k})|=\left|\mathbb{P}_{\delta_x}\exp(i\theta \langle  g_k,X_t\rangle)\right|\leq 1$ implying ${\rm Re~}(-w_{g_k})\geq0$,
From \eqref{eq: -v has positive real part} we know that $\operatorname{Re}(-v_{g_k})\geq 0$.
%we use the fact that
%\begin{align}
    %|x^{1+\beta}-y^{1+\beta}|
    %\leq C (|x|^{\beta}+|y|^{\beta})|x-y|\label{element},
%\end{align}
    %for $x,y \in \mathbb{C}$ such that ${\rm Re~} x \geq 0, {\rm Re~} y\geq 0$ to get
...
\end{align}
%Notice that, for $0\leq v \leq 1$, $x\in \mathbb{R}^d$ and $\theta \in \mathbb{R}$, by Lemma \ref{lemma1} and \eqref{eq:gcontrol}, we have
...
\label{eq: upper bound of wgk}
    %|w_{g_{k}}(x,v,\theta)|\leq &|\theta|e^{(\alpha-\kappa(g)b)k}(e^{(\alpha-\kappa(g)b)v}R(x))\leq|\theta|e^{(\alpha-\kappa(g)b)k}(C_1R(x)),
    |v_{g_{k}}(u,x,\theta)|
...
\end{align}
%deleted
    %where $C_1=\max\{1,e^{(\alpha-\kappa(g)b)}\}$.
%end deleted
%new added
...
\label{eq: Zineq}
   ...
   %\\\leq &C_1^{1+\beta}e^{\alpha}|\theta e^{(\alpha-\kappa(g)b)k}|^{1+\beta}R^{1+\beta}(x)\nonumber\\
   \\&\leq e^{\alpha}|C_g\theta e^{(\alpha-\kappa b)k+\alpha}|^{1+\beta}\tilde R^g_0(x).
%deleted
   %\\= & |\theta e^{(\alpha-\kappa(g)b)k}|^{1+\beta}R_2(x),
%end deleted
\end{align}
%where $R_2(x)=C_1^{1+\beta}e^{\alpha}R^{1+\beta}(x)$.
...
%end new added
    %\\&\leq |\theta e^{(\alpha-\kappa(g)b)k}|^{2+\beta}\cdot 2CC_1e^{\alpha}R(x)R_2(x).
    \\&\leq C_0e^{2\alpha}|C_g\theta e^{(\alpha-\kappa(g)b)k+\alpha}|^{1+2\beta}\tilde R^g_1(x).
    \qedhere
\end{align}
%delete
    %Take $p_1=2+\beta$ and $R_1(x)=2CC_1e^{\alpha}R(x)R_2(x)$, we obtain the desired result.
%end delete
\end{proof}
...
%end new
%For each $f\in \mathcal{R}$, define
For each $f\in \mathcal{P}$, define
\[
    Z_f(s,x,\theta)
    %:=\int_0^1 T^{\alpha}_{1-s}(-i\theta T_s^{\alpha}f)^{1+\beta}(x)ds ,
...
\label{cor: corollary2}
    %For any $k\in\{0,1,...,t\}$, $x\in \mathbb{R}^d$ and $\theta\in \mathbb{R}$, we have
...
\begin{align}
    |Z_{g_k}(s,x)|
    %&\leq \int_0^1 T_{1-s}^{\alpha}(|T_s^{\alpha}g_k|^{1+\beta})(x)ds \leq\int_0^t T_{1-s}^{\alpha}\left(|C_1 e^{(\alpha-\kappa(g)b)k}|R(x)\right )^{1+\beta}
...
\end{align}
%delete
    %The proof of \eqref{zineq1} is similar with \eqref{zineq2}.
%end delete
\end{proof}
...
%There exists $R_3 \in \mathcal{P}$ and $p_2 >1+\beta$ such that for any $k \in \mathbb{N}$ and $\theta \in \mathbb{R}$, we have
...
\[
    ...
    %=1+\langle Z_{g_k}(\cdot,\theta),\mu\rangle+\langle err(\cdot,\theta),\mu\rangle
...
\begin{proof}
    Let 
    % $ w_f(x,\theta)=w_f(x,1,\theta)$, $\tilde{w}_f(x,\theta)=\tilde{w}_f(x,1,\theta)$ for short, and write
\begin{align}
...
\end{align*}
%Using Lemma \ref{lemma2}, \eqref{eq: Zineq} and the fact that if $ {\rm Re~} z\leq 0$, $|e^z-1-z|\leq|z|^2$,   we get, for any $\theta \in \mathbb{R}$ and $k\in \mathbb{N}$,
...
%end new
    %\\&\leq |\theta_{k}|^{2(1+\beta)}\langle R_2,\mu\rangle + |\theta_{k}|^{2+\beta}\langle R_1,\mu\rangle,
...
\end{align*}
%deleted
    %where $\theta_k=\theta e^{(\alpha-\kappa(g)b)k}$. By implicitly assuming that $|\theta_k|\leq 1$, and taking $p_2=\min \{2(1+\beta), 2+\beta\}$ and $R_3=R_1+R_2$, we get the desired result.
%end deleted
\end{proof}
...
\end{lem}
\begin{proof}
    %Let $C(k)=e^{(\alpha-\kappa(g)b)k}$, and, for convenience, define $A:=\{|M_t^k[g]|>C(k)\}$, $A_{\mu}:=\{|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\mu\rangle|>C(k)\}$.
...
\begin{align*}
    II
    %=\mathbb{P}_{\mu}\left[\mathbb{P}_{\nu}\left[|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|^{1+\gamma}\mathbf{1}_{A_{\nu}}\right]|_{\nu=X_{t-k-1}}\right]
    =\mathbb{P}_{\mu}\big[\mathbb{P}_{\nu}[|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|^{1+\gamma}\mathbf{1}_{A_{k,\nu}}]|_{\nu=X_{t-k-1}}\big].
\end{align*}
%comment
{\bf Zhenyao: Just wondering why $X$ has compact support a.s.ly.}
%end comment
%Observe that, for any $\nu \in \mathcal{M}(\mathbb{R}^d)$
Observe that, for any $\nu \in \mathcal{M}(\mathbb{R}^d)$ with compact support, from Fubini's theorem,
\begin{align}
    %&\mathbb{P}_{\nu}\left[|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|^{1+\gamma}\right] \label{temp1}\\
    &\mathbb{P}_{\nu}[|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|^{1+\gamma} \mathbf 1_{A_{k,\nu}}] \label{temp1}\\
    %&=(1+\gamma)\int_0^{\infty}\lambda^{\gamma}\mathbb{P}_{\nu}\left(|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|\mathbf{1}_{A_{\nu}}>\lambda\right)d\lambda \nonumber\\
    &=(1+\gamma)\int_0^{\infty}\lambda^{\gamma}\mathbb{P}_{\nu}(|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|\mathbf{1}_{A_{k,\nu}}>\lambda)d\lambda \\
    &\leq (1+\gamma)\int_{C(k)}^{\infty}\lambda^{\gamma}\mathbb{P}_{\nu}\left(|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|>\lambda\right)d\lambda.\nonumber
\end{align}
%comment
    {\bf Zhenyao: Why the above inequality is true?}
%end comment
...
where $\varphi_{\nu}(\theta))$ has been defined in Corollary \eqref{cor: corollary1}. What's more, as $g_k=T_k^{\alpha}g$, by Corollary \eqref{cor: corollary1} and Corollary \eqref{cor: corollary2}, we get, for any $\theta\in\mathbb{R}$, there exists $p>\beta$ (actually, we just take $p=p_2-1$ ), such that
\begin{align*}
    |1-\varphi_{\nu}(\theta)|&\leq|\langle Z_{g_k}(\cdot,\theta),\nu \rangle|+|\langle err(\cdot,\theta),\nu \rangle|\\
    &\leq|\theta_k|^{1+\beta}\langle R_2,\nu\rangle+|\theta_k|^{1+p}\langle R_3, \nu \rangle\\
    &\leq (|\theta_k|^{1+\beta}+|\theta_k|^{1+p})\langle R_2+R_3, \nu \rangle.
\end{align*}
Therefore,
\begin{align*}
    &\mathbb{P}_{\nu}\big(|\langle T_k^{\alpha}g, X_1 \rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|>\lambda\big)\\
    &\leq\lambda\int_0^{2/\lambda}(|\theta_k|^{1+\beta}+|\theta_k|^{1+p})\langle R_2+R_3, \nu \rangle d\theta\\
    &= \frac{2^{2+\beta}}{2+\beta}e^{(\alpha-\kappa(g)b)k(1+\beta)}\lambda^{-(1+\beta)}+ \frac{2^{2+p}}{2+p}e^{(\alpha-\kappa(g)b)k(1+p)}\lambda^{-(1+p)})\langle R_2+R_3,\nu \rangle,
\end{align*}
 Then by calculating the integral in formula \eqref{temp1}, we get
\begin{align*}
    &\mathbb{P}_{\nu}\left[|\langle T_k^{\alpha}g, X_1\rangle-\langle T_{k+1}^{\alpha}g,\nu\rangle|^{1+\gamma} \right] \\
    &\leq C_3\left(e^{(\alpha-\kappa(g)b)k(1+\beta)}C(k)^{\gamma-\beta}+e^{(\alpha-\kappa(g)b)k(1+p)}C(k)^{\gamma-p}\right)\langle R_2+R_3,\nu\rangle\\
    &= C_3 e^{(\alpha-\kappa(g)b)k(1+\gamma)}\langle R_2+R_3,\nu\rangle,
\end{align*}
where $C_3=(1+\gamma)\cdot\max\{ \frac{2^{2+\beta}}{(\beta-\gamma)(2+\beta)}, \frac{2^{2+p}}{(2+p)(p-\gamma)}\}$. At last, using the mean formula \eqref{eq:meanformula}, we get
\begin{align*}
    II &\leq C_3e^{(\alpha-\kappa(g)b)k(1+\gamma)}\mathbb{P}_{\mu}\left[\langle R_2+R_3,X_{t-k-1}\rangle\right]\\
       &=C_3e^{-\alpha}\langle T_{t-k-1}(R_2+R_3),\mu \rangle e^{\alpha t}e^{(\gamma\alpha-\kappa(g)b(1+\gamma))k}\\
       &\leq C_4e^{\alpha t}e^{(\gamma\alpha-\kappa(g)b(1+\gamma))k}.
\end{align*}
 By \eqref{eq:semigroupineq}, there exists $R_4\in \mathcal{P}$, such that $T_t(R_2+R_3)\leq R_4$, we take $C_4=C_3 e^{-\alpha}\langle R_4, \mu\rangle$. Combining with the result of $I$ and $II$, we complete the proof by taking $C_2=(1+C_4)^{\frac{1}{1+\gamma}}$.
\end{proof}
\end{comment}
%end deleted
%new added
...
%For each $0\leq \gamma < \beta$ there exists a constant $C_5>0$, we have the result:
...
\label{eq: upper bound of psi'(v)}
    ...
    &\big|\psi'\big(x, -v_f(t,x,\theta)\big)\big|
    %\\&= \Big| -\beta(x)+ 2\alpha(x) v_f(t-s,x,\theta) + \int_{(0,\infty)} y (e^{-y v_f(t-s,x,\theta)} - 1) \pi(x,dy)\Big|
