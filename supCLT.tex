...
% A general  $f \in \mathcal P$ can be decomposed as $f_s + f_c + f_l$ with $f_s \in \mathcal C_s$, $f_c \in \mathcal C_c\setminus\{0\}$ and $f_l \in \mathcal C_l$. 
...
\begin{proof}
  ...
  and Lemma \ref{lem: property of controllable operators}, we get that, for all $\delta > 0$ small enough, the operators
  \[
    f
    \mapsto Z_t'' f
...
\begin{proof}
  ...
  \[
    |T_t f_1|
    % \leq e^{- \delta t}\sum_{p \in \mathbb Z_+^d : \tilde \beta \alpha > |p|b} |\langle f, \phi_p \rangle \phi_p|
  ...
  \begin{align}
    ...
    &\leq \int_0^1  \big\langle |(-iT_{rt}f)^{1+\beta} - (-if_c)^{1+\beta}|, \varphi\big \rangle ~dr 
      % = \langle g,\varphi\rangle\int_0^1 e^{-\delta rt} ~dr
...
\Big(  \frac{2^{2+\beta}}{2+\beta}\Big(\frac{e^{(t-s)(\alpha- \kappa b)}}{\lambda}\Big)^{1+\beta} + \frac{2^{3}}{3}\Big(\frac{e^{(t-s)(\alpha- \kappa b)}}{\lambda}\Big)^2\Big)
    \\ & \leq C e^{\alpha r} \Big(\Big( \frac{e^{(t-s)(\alpha - \kappa b)}}{\lambda}\Big)^{1+\beta} + \Big( \frac{e^{(t-s)(\alpha - \kappa b)}}{\lambda}\Big)^{2} \Big),
  \end{align}
  % where the $C>0$ above is chosen as $ C := \Big(\frac{2^{2+\beta}}{2+\beta} + \frac{2^{3}}{3} \Big) \mu(Q_0Rh).\qedhere $
