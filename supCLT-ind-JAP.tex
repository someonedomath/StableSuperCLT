...
%We will write $H^0_t$ and $H^0_\infty$ as $H^0_t$ and $H_\infty$, respectively.
...
%	a similar argument used in the proof of 
...
%\eqref{e:Tt} and $\mathrm x_t(f_\mathrm l)$ is defined in \eqref{eq:oldResult} 
...
%	Theorem 1.6 in \cite{RenSongSunZhao2019Stable}, and the fact that the convergence in
...
%	taking as $\tilde{f_\mathrm s}+ \tilde{f}_{\mathrm c}$ and $g$ taking as 
taken as $\tilde{f_\mathrm s}+ \tilde{f}_{\mathrm c}$ and $g$ taken as
	$\tilde{f}_{\mathrm l}$
	%, and get that there exist $C_1,\delta_1 > 0$ such that for every $t>e$ (which implies $t\geq \lfloor t - \ln t\rfloor +1$),
