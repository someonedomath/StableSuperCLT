...
%\end{align}
	...
	And for each $p\in \mathbb Z_+^d$, $\phi_p$ corresponding to eigenvalue $-b|p|$ where $|p|:= \sum_{k=1}^d p_k$.
	% It is known that $(P_t)_{t\geq 0}$ is a strongly continuous semigroup in $L^2(\varphi)$ and its generator $L$ has discrete spectrum $\sigma(L)= \{-bk: k \in \mathbb Z_+\}$.
...
%\end{align}
	%is an eigenfunction of $L$ corresponding to the eigenvalue $b|p|$.
...
%{
	...
	We will write $H^0_t$ and $H^0_\infty$ as $H^0_t$ and $H_\infty$.
%}
...
\begin{align}\label{e:Tt}
	...
	:= \sum_{p \in \mathbb Z_+^d} \exp\{-\big| |p|b - \alpha \tilde \beta \big|t\} \langle f, \phi_p \rangle_{\varphi} \phi_p,
...
%{and if $f\in  \mathcal P$ satisfies $\alpha \tilde \beta > \kappa_f b$, then $f_\mathrm c$ and $f_\mathrm l$ maybe non-zero.
...
%}
	...
	The main purpose of this note is to confirm this conjecture. %}
...
\]
	...
	The following Lemma \ref{lem:P:R} can be proved using a similar argument used in the proof of \cite[Lemma 2.9]{RenSongSunZhao2019Stable}. We omit the details here.
\begin{lem}
\label{lem:P:R}
	%{Suppose that $g = g_\mathrm s + g_\mathrm c + g_\mathrm l$ with $g_\mathrm s \in \mathcal C_\mathrm s, g_\mathrm c \in \mathcal C_\mathrm c$ and $g_\mathrm 1 \in \mathcal C_\mathrm l$.
	%Then there exists $h \in \mathcal P^+$ such that for all $ f \in \overline {\mathcal P}_g$ and $t\geq 0$, we have $ | P_t (Z_1 f - \langle Z_1 f, \varphi \rangle )| \leq e^{-bt} h$.
...
\end{lem}
	...
	%{with using Lemma \ref{lem:P:R} instead of Lemma 2.9 in \cite{RenSongSunZhao2019Stable}.
	with using Lemma \ref{lem:P:R} above instead of Lemma 2.9 in \cite{RenSongSunZhao2019Stable}.
	Let $(\mathscr F_t)_{t\geq 0}$ be the natural filtration of $X$.
	%}
\begin{prop}
\label{thm:Key}
	For each $\mu \in \mathcal M_c(\mathbb R^d)$  and $g \in \mathcal P$, there exist $C,\delta>0$ such that for all $t\geq 1$ and $f \in \overline{\mathcal P}_g$, we have
\[
	\mathbb P_\mu
...
\begin{prop} \label{cor:MI}
	For each  $f,g\in \mathcal P$ and $\mu\in \mathcal M_c(\mathbb R^d)$, there exist $C,\delta>0$ such that for all $n_1,n_2 \in \mathbb Z_+$, $(f_j)_{j=0}^{n_1}\subset \overline{\mathcal P}_f$, $(g_j)_{j=0}^{n_2}\subset \overline{\mathcal P}_g$ and $t\geq n_1+1$, we have
\begin{equation}
\label{32corollary}
...
%{Let $(\mathscr F_t)_{t\ge 0}$ be the natural filtration of $X$. %}
...
\end{comment}
	...
	In the remainder of this section, we always fix $\mu \in \mathcal M_c(\mathbb R^d)\setminus\{0\}$, $f_\mathrm s\in \mathcal C_\mathrm s$,  $f_\mathrm c\in \mathcal C_\mathrm c$ and $f_\mathrm l\in \mathcal C_\mathrm l$.
...
\end{align}
%{
	%where $T_k$ is the operator defined in \eqref{e:Tt} and $\mathrm x_t(f_\mathrm l)$ is defined in \eqref{eq:oldResult} with $f$ replaced with $f_\mathrm l$, $\tilde f_\mathrm s:=e^{\alpha(\tilde \beta - 1)} f_\mathrm s$, $\tilde f_\mathrm c:=e^{\alpha(\tilde \beta - 1)} f_\mathrm c$ and $\tilde f_\mathrm l := \sum_{p\in \mathbb Z^d_+: \alpha\tilde\beta>|p|b}e^{-(\alpha - |p|b)}\langle f_\mathrm l, \phi_p \rangle_\varphi \phi_p$.
	where $T_k$ is the operator defined in \eqref{e:Tt}, $\tilde f_\mathrm s:=e^{\alpha(\tilde \beta - 1)} f_\mathrm s$, $\tilde f_\mathrm c:=e^{\alpha(\tilde \beta - 1)} f_\mathrm c$ and $\tilde f_\mathrm l := \sum_{p\in \mathbb Z^d_+: \alpha\tilde\beta>|p|b}e^{-(\alpha - |p|b)}\langle f_\mathrm l, \phi_p \rangle_\varphi \phi_p$.
%}
...
\[
	...
	\widetilde{\mathbb P}_{\mu}\Big[\exp\Big(i \sum_{j=\mathrm s,\mathrm c,\mathrm l} R_j(t)\Big)\Big] %}
...
\]
	...
	Therefore, we can use Proposition \ref{cor:MI} with $f$ taking as $\tilde{f_\mathrm s}+ \tilde{f}_{\mathrm c}$ and $g$ taking as $\tilde{f}_{\mathrm l}$, and get that there exist $C_1,\delta_1 > 0$ such that for every $t>e$ (which implies $t\geq \lfloor t - \ln t\rfloor +1$),
\begin{align}
    %{&\Big|\widetilde{\mathbb P}_{\mu}\Big[\exp\Big(i \sum_{j=\mathrm s,\mathrm c,\mathrm l}I_0^{f_j}(t)\Big)\Big] -\exp\Big(\sum_{k=0}^{\lfloor t-\ln t \rfloor} \langle Z_1(T_{k}(\tilde f_\mathrm s+t^{\tilde{\beta}-1}\tilde{f}_{\mathrm c})), \varphi\rangle \Big)\exp\Big(\sum_{k=0}^{\lfloor t^2 \rfloor}\langle Z_1(-T_k\tilde{f}_{\mathrm l}),\varphi\rangle\Big)\Big|
    &\begin{multlined} 
