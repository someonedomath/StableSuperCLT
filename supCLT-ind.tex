...
\end{align}
%{
	Let us explain the non-integer power of the complex numbers used above. 
...
%{
  ...
  %and if $f\in  \mathcal P$ satisfies $\alpha \tilde \beta > \kappa_f b$, then $f_\mathrm s$ and $f_\mathrm l$ maybe non-zero while $f_\mathrm c \equiv 0$.
	%In \cite{RenSongSunZhao2019Stable}, we were not able to establish a central limit theorem in this case. 
	Any  $f \in \mathcal P$ can be decomposed as $f_s + f_c + f_l$ with $f_s \in \mathcal C_\mathrm s$, $f_c \in \mathcal C_\mathrm c$ and $f_l \in \mathcal C_\mathrm l$. 
...
\begin{rem}
	...
	Note that in this case if $l$ blows up at $0$ (for example $l(z) = \log z$) our result can not hold anymore. 
	This is simply because the limiting $\alpha$-stable random variable $\zeta^f$ described in \eqref{eq:I:R:3} will cease to make sense. 
\end{rem}
...
\begin{proof}
   	...
   	Also note that $\overline{ z^{1+\beta}} = \overline z^{1+\beta}$ for each $z \in \mathbb C \setminus (-\infty, 0)$ where $\bar z$ represents the conjugation of $z$.
   	So by taking conjugation, we can verify that 
\[
	|(ix+iy)^{1+\beta}-(ix)^{1+\beta}-(iy)^{1+\beta}|
	= |(-ix-iy)^{1+\beta}-(-ix)^{1+\beta}-(-iy)^{1+\beta}|.
\]
	This allows us to further assume without loss of generality that $y>0$.
	Now let us calculate that for each $z\in \mathbb C$, 
\begin{align}
...
\end{align}
	Taking absolute value and using continuity, we get that for each $z\in \mathbb C$ there exists $C(z)>0$ such that for each $r\geq 1$,
\[
  	|(r\pm 1)^{1+\beta}-r^{1+\beta} - z|
  	\leq C(z) |r|^{\beta}.
\]
	For any $u\in \mathbb R\setminus\{0\}, r>0$ and $\gamma \geq 0$, from the fact that $\arg (iu r) = \arg(iu)+ \arg(r)$ we can verify $(iur)^{\gamma} = (iu)^{\gamma} r^{\gamma}$.
...
\end{align}
	To summarize, there exists a $C_1>0$ such that for each $r\in \mathbb R$ with $|r|\geq 1$,
\[
...
\begin{align}
	&|(ix+iy)^{1+\beta}-(ix)^{1+\beta}-(iy)^{1+\beta}|
	= \big|y^{1+\beta}\big(i(1+x/y)^{1+\beta}-(ix/y)^{1+\beta}- i^{1+\beta}\big)\big|
...
\begin{multlined}
	...
	%{
		%e^{(\alpha - |p|b)(t+r)} e^{-(\alpha - |p|b)u}X_u(\phi_p) 
