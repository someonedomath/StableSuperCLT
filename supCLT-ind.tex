...
%	Let us explain the non-integer power of the complex numbers used above. 
	The non-integer power of the complex numbers above is has its usual meaning. For the reader's convenience, we recall the its definition here.
%}	
...
\end{align}
%{
%	Taking absolute value and using continuity, 
Combining the above with continuity, 
	%}
	we get that for each $z\in \mathbb C$ there exists $C(z)>0$ such that for each $r\geq 1$,
\[
...
%}	
	there exists a $C_1>0$ such that for each $r\in \mathbb R$ with $|r|\geq 1$,
\[
...
{\bf Acknowledgments:}  We thank the referees for the very helpful comments.
%
